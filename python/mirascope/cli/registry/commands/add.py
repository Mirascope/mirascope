...
def add_command(
    ...
    for item_name in items:
        ...
        if (
            ...
            if item is None:
                typer.echo(f"Error: Local file '{item_name}' not found.", err=True)
                raise typer.Exit(1)
        else:
            try:
                item = client.fetch_item(item_name, language="python")
            except Exception as e:
                typer.echo(f"Error: Failed to fetch '{item_name}': {e}", err=True)
                raise typer.Exit(1)

            if item is None:
                typer.echo(
                    f"Error: Item '{item_name}' not found in registry.", err=True
                )
                raise typer.Exit(1)
        ...
        for file_info in item.get("files", []):
            ...
            try:
                ...
                typer.echo(f"Error: Failed to write {target_path}: {e}", err=True)
                raise typer.Exit(1)
