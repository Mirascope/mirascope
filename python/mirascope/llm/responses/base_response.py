...
class BaseResponse(Generic[FormatT]):
    ...
    def pretty(self, include_content_placeholders: bool = True) -> str:
        """Return a string representation of all response content.

        If include_content_placeholders is True, includes placeholder representations
        for all content types (tool calls, thinking, images, etc) along with text.
        If False, returns only text content separated by newlines.
        """
        raise NotImplementedError()
