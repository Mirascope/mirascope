...
def _wrap_target(
    ...
    if isinstance(target, AsyncCall):
        ...
        return AsyncRetryCall(
            ...
            prompt=AsyncRetryPrompt(
                fn=prompt.fn, toolkit=prompt.toolkit, format=prompt.format, retry_config=retry_config
            ),
        )
    ...
    if isinstance(target, AsyncPrompt):
        return AsyncRetryPrompt(
            fn=target.fn, toolkit=target.toolkit, format=target.format, retry_config=retry_config
        )
