...
async def _encode_message_async(
    message: UserMessage | AssistantMessage,
    model_id: AnthropicModelId,
    encode_thoughts: bool,
    provider: Literal["anthropic", "anthropic-bedrock", "anthropic-vertex"],
    ...
    return {
        "role": message.role,
        "content": await _encode_content_async(
            message.content, encode_thoughts, provider
        ),
    }
    ...
    if format is not None:
        if format.mode == "strict":
            raise FormattingModeNotSupportedError(
                formatting_mode="strict", provider=provider, model_id=model_id
            )
...
async def encode_request_async(
    *,
    ...
    encoded_messages = []
    for remaining_message in remaining_messages:
        encoded_message = await _encode_message_async(
            remaining_message, model_id, encode_thoughts, provider
        )
