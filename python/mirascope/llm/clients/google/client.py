...
from ..base import BaseClient, _extract_system_message
...
class GoogleClient(BaseClient[GoogleParams, GoogleModel]):
    ...
    def call(
        ...
        *,
        ...
        system_message, non_system_messages = _extract_system_message(messages)
        ...
        google_response = self.client.models.generate_content(
            model=model,
            contents=_encode_messages(non_system_messages),
            config=config,
        )

        assistant_message, finish_reason = _decode_response(google_response)
