...
def test_azure_provider_lazy_init_client() -> None:
    """Test AzureProvider lazily initializes the OpenAI client."""
    provider = AzureProvider(
        api_key="test-openai-key",
        base_url="https://test-resource.openai.azure.com/",
    )

    # Client should be None before any subprovider is accessed
    assert provider.client is None

    # Access OpenAI subprovider - this should initialize the client
    openai_provider = provider._get_openai_provider()  # pyright: ignore[reportPrivateUsage]
