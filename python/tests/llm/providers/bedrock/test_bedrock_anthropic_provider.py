...
from mirascope.llm.providers.bedrock._utils import bedrock_model_name
...
def test_bedrock_anthropic_provider_initialization_with_credentials(
...
def test_bedrock_anthropic_provider_initialization_with_api_key_env(
    ...
    """Test BedrockAnthropicProvider uses AWS_BEARER_TOKEN_BEDROCK env var."""
...
def test_bedrock_anthropic_provider_api_key_env_var_class_attribute() -> None:
    ...
    assert BedrockAnthropicProvider.api_key_env_var == "AWS_BEARER_TOKEN_BEDROCK"
...
def test_bedrock_anthropic_provider_sigv4_uses_anthropic_bedrock_client(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test BedrockAnthropicProvider uses AnthropicBedrock for SigV4 auth.

    When no API key is provided, the provider uses the AnthropicBedrock client
    which handles AWS credential resolution internally.
    """
    ...
    # Verify that the provider uses AnthropicBedrock (not the API key client)
    assert isinstance(provider.client, AnthropicBedrock)
    # Verify it's not the API key subclass
    assert type(provider.client).__name__ == "AnthropicBedrock"
