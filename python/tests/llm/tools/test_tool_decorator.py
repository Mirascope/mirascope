...
from mirascope.llm.content import ToolCall, ToolOutput
from mirascope.llm.tools.tool import AsyncTool, Tool
from mirascope.llm.tools.tool_decorator import tool
...
def test_sync_tool_basic() -> None:
    ...
    tool_call = ToolCall(id="call_123", name="add_numbers", args={"a": 5, "b": 7})
    output = add_numbers.execute(tool_call)
    expected = ToolOutput(id="call_123", value=12)
...
def test_tool_decorator_syntax() -> None:
    ...
    assert isinstance(without_parens, Tool)
    assert without_parens.name == "without_parens"
    assert without_parens() == "simple"

    assert isinstance(with_parens, Tool)
...
def test_decorator_modes_equivalence() -> None:
    ...
    decorated = tool(base_func)
    via_factory = tool()(base_func)

    # Should be equal Tool instances
