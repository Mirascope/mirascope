...
class TestStreamFallbackModels:
    """Tests for stream fallback model functionality."""
    ...
    def test_stream_fallback_model_gets_own_retry_budget(
        ...
        assert exc_info.value.attempt == 1
        assert (
            response.model.model_id == "mock/primary"
        )  # Still on primary (has retries)
        ...
        assert (
            response.model.model_id == "mock/fallback"
        )  # Still on fallback (has retries)
...
@pytest.mark.asyncio
class TestAsyncStreamFallbackModels:
    """Tests for async stream fallback model functionality."""
