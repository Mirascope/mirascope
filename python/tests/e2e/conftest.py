...
def _custom_body_matcher(cassette_req: VCRRequest, incoming_req: VCRRequest) -> bool:
    """Custom body matcher that skips OAuth and Google Gemini requests."""

    if (
        "oauth2.googleapis.com/token" in cassette_req.uri
        or "generativelanguage.googleapis.com" in cassette_req.uri
    ):
        return True

    return cassette_req.body == incoming_req.body
...
def _register_custom_matchers() -> None:
    ...
    Also normalizes Google API thoughtSignature fields to handle base64 vs
    base64url encoding variations between responses and requests.
    """
    ...
    def patched_init(self: VCR, *args: Any, **kwargs: Any) -> None:  # noqa: ANN401
        ...
        self.matchers["body"] = _custom_body_matcher  # type: ignore[assignment]
