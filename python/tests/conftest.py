...
def gcp_credentials_file() -> Generator[str, None, None]:
    ...
    SECURITY NOTE: All credentials below are dummy/fake values for testing only:
    - The project_id, client_email, and key_id are clearly marked as "dummy"
    - The RSA private key is a randomly generated test key with no real-world use
    - This file is only used in CI when no real credentials exist
    - VCR cassettes replay HTTP responses, so this key is never actually used for signing
    ...
    credentials = {
        ...
        # This is a randomly generated RSA key for testing purposes only.
        # It has no real-world use and is never used to sign actual requests.
        # The key format must be valid PEM because google.auth validates the structure.
...
def load_api_keys(gcp_credentials_file: str) -> None:
    ...
    # Google Vertex AI credentials: prefer ~/.config/gcloud/application_default_credentials.json
    # Only set dummy credentials if real ADC doesn't exist
    ...
    # Set defaults for region and GCE check
