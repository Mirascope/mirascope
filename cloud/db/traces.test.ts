...
describe("Traces helper functions", () => {
  describe("convertOTLPValue", () => {
    ...
    it("handles arrayValue with primitive items", () => {
      // When items are not objects, they pass through unchanged
      const value = {
        arrayValue: {
          values: ["string", 42, null] as unknown as OTLPValue[],
        },
      } as unknown as OTLPValue;
      expect(convertOTLPValue(value)).toEqual(["string", 42, null]);
    });
    ...
    it("handles kvlistValue with primitive values", () => {
      // When kv.value is not an object, it passes through unchanged
      const value = {
        kvlistValue: {
          values: [
            { key: "primitive", value: "string" },
            { key: "number", value: 42 },
            { key: "null", value: null },
          ],
        },
