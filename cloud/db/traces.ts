      ...
      yield* client
        ...
        .pipe(
          Effect.mapError(
            (e) =>
              new DatabaseError({
                message: `Failed to delete spans: ${e instanceof Error ? e.message : "Unknown error"}`,
                cause: e,
              }),
          ),
        );
      ...
      const result: { id: string }[] = yield* client
        ...
        .pipe(
          Effect.mapError(
            (e) =>
              new DatabaseError({
                message: `Failed to delete trace: ${e instanceof Error ? e.message : "Unknown error"}`,
                cause: e,
              }),
          ),
        );
        ...
        for (const scopeSpan of rs.scopeSpans) {
          for (const span of scopeSpan.spans) {
            ...
            const result = yield* Effect.gen(function* () {
              ...
              const upsertResult: { id: string }[] = yield* client
                .insert(traces)
                .values(traceData)
                .onConflictDoUpdate({
                ...
                .pipe(
                  Effect.mapError(
                    (e) =>
                      new DatabaseError({
                        message: `Failed to upsert trace: ${e instanceof Error ? e.message : "Unknown error"}`,
                        cause: e,
                      }),
                  ),
                );

              const upsertedTrace = upsertResult[0];
              ...
              const insertedSpans: { id: string }[] = yield* client
                ...
                .pipe(
                  Effect.mapError(
                    (e) =>
                      new DatabaseError({
                        message: `Failed to insert span: ${e instanceof Error ? e.message : "Unknown error"}`,
                        cause: e,
                      }),
                  ),
                );
