...
import type { DrizzleORM } from "@/db/client";
import type {
  CreateAnnotationInput,
  UpdateAnnotationInput,
} from "@/db/annotations";

type DatabaseService = Context.Tag.Service<Database>;

type AnnotationsInternal = {
  findSpan: (
    spanId: string,
    traceId: string,
    environmentId: string,
  ) => Effect.Effect<unknown, unknown, DrizzleORM>;
  createInternal: (
  ...
  deleteInternal: (
    id: string,
    environmentId: string,
  ) => Effect.Effect<void, unknown, DrizzleORM>;
  getByIdInternal: (
    id: string,
    environmentId: string,
  ) => Effect.Effect<unknown, unknown, DrizzleORM>;
  listInternal: (
    ...
    { annotations: unknown[]; total: number },
    DatabaseError,
    DrizzleORM
  >;
};

const getAnnotationsInternal = (db: DatabaseService): AnnotationsInternal =>
  db.organizations.projects.environments
    .annotations as unknown as AnnotationsInternal;
...
describe("Annotations", () => {
  ...
  describe("findAll/findById", () => {
    it.effect("findAll returns annotations without pagination wrapper", () =>
      Effect.gen(function* () {
        const { environment, project, org, owner } =
          yield* TestEnvironmentFixture;
        const db = yield* Database;
        ...
        yield* db.organizations.projects.environments.annotations.create({
          ...
          projectId: project.id,
          environmentId: environment.id,
          data: { spanId, traceId, label: "find-all" },
        });

        const result =
          yield* db.organizations.projects.environments.annotations.findAll({
            userId: owner.id,
            organizationId: org.id,
            projectId: project.id,
            environmentId: environment.id,
        ...
        expect(result[0].label).toBe("find-all");
      }),
    );

    it.effect("findById returns annotation", () =>
      Effect.gen(function* () {
        const { environment, project, org, owner } =
          yield* TestEnvironmentFixture;
        const db = yield* Database;
        ...
        const created =
          yield* db.organizations.projects.environments.annotations.create({
            ...
            projectId: project.id,
            environmentId: environment.id,
            data: { spanId, traceId, label: "find-by-id" },
          });

        const result =
          yield* db.organizations.projects.environments.annotations.findById({
            userId: owner.id,
            organizationId: org.id,
            projectId: project.id,
            environmentId: environment.id,
        ...
        expect(result.label).toBe("find-by-id");
      }),
    );
  });

  describe("internal error mapping", () => {
    it.effect("findSpan returns DatabaseError on query failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);

        const result = yield* annotationsService
          .findSpan(
            "0123456789abcdef",
        ...
        Effect.provide(
          new MockDrizzleORM().select(new Error("Select failed")).build(),
        ),
      ),
    );

    it.effect(
      "createInternal returns AlreadyExistsError on unique violation",
      () => {
    ...
    it.effect("createInternal returns DatabaseError on insert failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);
    ...
    it.effect("updateInternal returns DatabaseError on update failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);
    ...
    it.effect("deleteInternal returns DatabaseError on delete failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);
    ...
    it.effect("getByIdInternal returns DatabaseError on query failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);
    ...
    it.effect("listInternal handles empty count result", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);

        const result = yield* annotationsService.listInternal(
          "00000000-0000-0000-0000-000000000001",
        );
        ...
        Effect.provide(new MockDrizzleORM().select([]).select([]).build()),
      ),
    );

    it.effect("listInternal returns DatabaseError on count failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);

        const result = yield* annotationsService
          .listInternal("00000000-0000-0000-0000-000000000001")
          .pipe(Effect.flip);

        expect(result).toBeInstanceOf(DatabaseError);
      }).pipe(
        Effect.provide(
          new MockDrizzleORM().select(new Error("Count failed")).build(),
        ),
      ),
    );

    it.effect("listInternal returns DatabaseError on list failure", () =>
      Effect.gen(function* () {
        const db = yield* Database;
        const annotationsService = getAnnotationsInternal(db);
