...
description: Techniques for evaluating SQL agent performance. This guide demonstrates methods to assess query accuracy, SQL correctness, and result relevance.
---

# Evaluating Generating SQL with LLM

In this recipe, we will be using taking our [SQL Agent](/docs/v1/guides/agents/sql-agent) example and running evaluations on LLM call. We will be exploring various different evaluations we can run to ensure quality and expected behavior.
...
<Note title="Check out the SQL Agent Tutorial">
We will be using our `LibrarianAgent` for our evaluations. For a detailed explanation regarding this code snippet, refer to the [SQL Agent Tutorial](/docs/v1/guides/agents/sql-agent).
</Note>
