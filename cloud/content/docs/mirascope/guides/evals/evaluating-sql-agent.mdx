...
In this recipe, we will be using taking our [SQL Agent](/docs/mirascope/guides/agents/sql-agent) example and running evaluations on LLM call. We will be exploring various different evaluations we can run to ensure quality and expected behavior.
...
We will be using our `LibrarianAgent` for our evaluations. For a detailed explanation regarding this code snippet, refer to the [SQL Agent Tutorial](/docs/mirascope/guides/agents/sql-agent).
</Note>
