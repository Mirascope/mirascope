...
When working programmatically with LLMs, we often want reusable functions that encapsulate constructing messages and getting an LLM response. Using just what we've already learned about [Messages](/docs/llm/messages) and [Models](/docs/llm/models), we might write code like this:

<CodeExample file="python/examples/prompts/basic_without_prompt.py" />
...
<CodeExample file="python/examples/prompts/return_types.py" lines="3-" />
...
<CodeExample file="python/examples/prompts/with_params.py" lines="3-" />
...
<CodeExample file="python/examples/prompts/messages_method.py" lines="3-" />
...
| --- | --- |
| `tools` | List of tools the LLM can call. See [Tools](/docs/llm/tools). |
| `format` | Response format for structured output. See [Structured Output](/docs/llm/structured-output). |
...
- [Calls](/docs/llm/calls) — Bundle a model with your prompt function
- [Tools](/docs/llm/tools) — Let LLMs call functions
- [Structured Output](/docs/llm/structured-output) — Parse responses into structured data
