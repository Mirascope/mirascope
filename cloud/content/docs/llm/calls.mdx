...
In the [Prompts](/docs/llm/prompts) guide, we talked about how the `@llm.prompt` decorator streamlines writing LLM powered functions. 
...
`@llm.call` does this via `llm.use_model()`, which can be used to manually similar behavior. See [Models](/docs/llm/models).
</Note>
...
| `temperature`, `max_tokens`, etc. | Model parameters. See [Models](/docs/llm/models). |
