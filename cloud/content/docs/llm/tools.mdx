...
When any tool is async, the call must be async too, and you use `await` with `execute_tools()` and `resume()`. See the [Async](/docs/llm/async) guide for more details.
...
- [Streaming](/docs/llm/streaming) — Stream responses with tool calls
- [Context](/docs/llm/context) — Inject dependencies into tools
- [Agents](/docs/llm/agents) — Build autonomous agents with tools
