...
(That’s why we favor the simplicity of a [Pythonic approach](/docs/mirascope/getting-started/why/), like Mirascope’s, which leverages Python’s native syntax to allow fine-grained customization of each pipeline stage.)
...
Mirascope’s call decorator (`@openai.call`) readily transforms regular Python functions into prompts by converting them into API requests. It also [colocates calls](/blog/engineers-should-handle-prompting-llms/) with prompts to make development and maintenance easier.
