...
Popular techniques like [Chain-of-thought (CoT)](/docs/mirascope/guides/prompt-engineering/text-based/chain-of-thought/) and Tree-of-thought (ToT) prompting or decision trees can help structure this process - enabling the agent to think through problems step by step, especially when dealing with complex multi-step tasks.  
...
Mirascope lets you build autonomous or semi-autonomous systems that handle tasks, make decisions, and interact with users to integrate with tools, manage state, and even bring [humans into the loop](/docs/mirascope/learn/agents/) when needed.
...
If the function doesn’t contain a docstring or can’t be changed for some reason (like if it’s third-party code) then you can use Mirascope’s [`BaseTool` class](/docs/mirascope/learn/tools/#__tabbed_2_1) to define the tool.
...
You can find more details about the code below in our [tutorial library](/docs/mirascope/guides/agents/local-chat-with-codebase/).
