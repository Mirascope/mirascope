...
describe("MockCloudflareR2Service", () => {
  ...
  describe("listBuckets", () => {
    ...
    it("filters by prefix using startsWith", async () => {
      const buckets = await run(
        Effect.gen(function* () {
          const r2 = yield* CloudflareR2Service;
          yield* r2.createBucket("claw-aaa");
      ...
      expect(buckets.every((b) => b.name.startsWith("claw-"))).toBe(true);
    });

    it("does not match prefix in the middle of bucket names", async () => {
      const buckets = await run(
        Effect.gen(function* () {
          const r2 = yield* CloudflareR2Service;
          yield* r2.createBucket("claw-aaa");
      ...
      expect(buckets[0].name).toBe("claw-aaa");
    });
