...
if [[ -n "${CLICKHOUSE_MIGRATE_DATABASE_URL:-}" ]]; then
  ...
  database_url="clickhouse://${host}:${migrate_port}/${clickhouse_database}?username=${clickhouse_user}&password=${clickhouse_password}&x-multi-statement=true&x-migrations-table=clickhouse_migrations&x-migrations-table-engine=MergeTree"
fi
