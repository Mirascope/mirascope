...
interface GitHubMirascopeButtonProps {
  className?: string;
}

const GitHubMirascopeButton: React.FC<GitHubMirascopeButtonProps> = ({
  className,
}) => {
  const { github } = MIRASCOPE;
  if (!github) {
    return null;
  }

  const { repo, stars, version } = github;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 text-base font-medium">
        {GitHubIcon}
        <span>{MIRASCOPE.title.toLowerCase()}</span>
      </div>
...
export default GitHubMirascopeButton;
