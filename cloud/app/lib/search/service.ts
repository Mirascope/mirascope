...
export class PagefindSearchService implements SearchService {
  ...
  async init(): Promise<void> {
    ...
    try {
      // Load content metadata synchronously, then load Pagefind
      this.loadContentMeta();
  ...
  private async initPagefind(): Promise<void> {
    ...
    try {
      // Dynamic import is necessary for Pagefind since it's generated at build time
      // and the path isn't known at compile time. Using Function constructor to bypass
      // static analysis since Pagefind generates its own module system.
  ...
  private loadContentMeta(): void {
    try {
      ...
      this.contentMeta = getAllContentMeta();
