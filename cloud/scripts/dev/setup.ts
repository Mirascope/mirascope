...
async function ensureContainers() {
  ...
  if (pgRunning && chRunning) {
    console.log("🐘 Postgres + ClickHouse already running");
    return;
  }
...
function ensureEnvLocal() {
  ...
  console.log("📄 Creating .env.local with dev defaults...");
...
# Local development environment — all values are dev-safe defaults.

ENVIRONMENT=development
DATABASE_URL=${DB_URL}
SITE_URL=http://localhost:5173

# Auth (GitHub OAuth — register a dev app at https://github.com/settings/developers)
...
# Analytics (PostHog + GA — not needed locally)
...
# OpenClaw gateway (for local WS proxy — connects chat UI to your local gateway)
...
`;
  ...
  console.log(
    "   ⚠️  Edit OPENCLAW_GATEWAY_WS_URL and OPENCLAW_GATEWAY_TOKEN to enable chat",
  );
}
...
async function printReadyMessage() {
  ...
  console.log("");
  console.log("Auth is automatic — no login needed.");
