...
function isAroundSunsetTime(): boolean {
  ...
  if (countryCode === "_") {
    ...
    if (browserLang && browserLang.includes("-")) {
      ...
      if (REGION_LATITUDES[langCountry]) {
  ...
  // Real sunset time detection
  isSunsetTime =
    (hour === sunsetHourInt && minutes >= sunsetMinute - 30 && minutes <= 59) ||
    (hour === sunsetHourInt + 1 && minutes <= (sunsetMinute + 30) % 60);
