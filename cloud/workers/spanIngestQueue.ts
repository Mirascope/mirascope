 ...
 * prioritized; DO upsert is best-effort and failures are logged but don't block
 * the ingestion. Failed ClickHouse inserts are retried via the queue mechanism.
 */
 ...
 *
 * Writes to ClickHouse (source of truth) and updates the realtime cache (DO).
 * DO upsert is best-effort: failures are logged but don't block ClickHouse insert.
 * This ensures data durability even when the DO is temporarily unavailable.
 *
...
type SpanIngestEnvironment = CloudflareEnvironment & {
  readonly RECENT_SPANS_DO?: DurableObjectNamespace;
};
...
export default {
  ...
  async queue(
    ...
    /* v8 ignore start - no-op layer when DO unavailable, covered by unit tests */
    const realtimeLayer = env.RECENT_SPANS_DO
      ? RealtimeSpansLive(env.RECENT_SPANS_DO)
