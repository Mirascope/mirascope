...
def test_template():
    """Test that `BasePrompt` initializes properly."""
    assert (
...
@patch.object(BasePrompt, "template", return_value="{foo}{bar}")
...
def test_save_and_load(fixture_foobar_prompt, tmpdir):
    """Test that `BasePrompt` can be saved and loaded."""
...
def test_prompt_dump(
    ...
    """Tests that `BasePrompt.dump` returns the expected string."""
...
class NoDocstringPrompt(BasePrompt):
    param: str


def test_no_docstr():
    """Tests that `BasePrompt` throws a value error if it doesn't have a docstring."""
    with pytest.raises(ValueError):
