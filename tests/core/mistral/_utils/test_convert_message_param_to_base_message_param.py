...
def test_mistral_convert_content_text_only(content_str: str):
    ...
    """
    result = convert_message_param_to_base_message_param(Mock(content=content_str))
...
def test_mistral_convert_content_text_chunk():
    ...
    with patch(
        ...
        result = convert_message_param_to_base_message_param(Mock(content=content))  # pyright: ignore [reportArgumentType]
    assert isinstance(result, BaseMessageParam)
