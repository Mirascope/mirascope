"""Tests the `azureai._utils.convert_message_params` function."""
...
from mirascope.core.azureai._utils._convert_message_params import convert_message_params
...
def test_convert_message_params() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Unsupported image media type: image/svg. AzureAI currently only supports "
    ...
    with pytest.raises(
        ValueError,
        match="AzureAI currently only supports text and image parts. "
        "Part provided: audio",
    ):
        convert_message_params(
            [
