...
<Note>
`tool_calls` and `thoughts` are populated only when relevant. See [Tools](/docs/tools) and [Thinking](/docs/thinking) for details.
</Note>
...
- **Streaming:** The response content gets populated by iterating through the stream. See [Streaming](/docs/streaming).
- **Async:** Methods like `resume()` are async. See [Async](/docs/async).
- **Context:** Methods like `resume()` require a context argument. See [Context](/docs/context).
...
When the response was generated with a `format` parameter, use `response.parse()` to extract structured data. See [Structured Output](/docs/structured-output) for the full guide on structured output.

## Next Steps

Now that you understand responses, explore:

- [Streaming](/docs/streaming) — Process responses as they're generated
- [Tools](/docs/tools) — Let LLMs call your functions
- [Structured Output](/docs/structured-output) — Extract structured data from responses
