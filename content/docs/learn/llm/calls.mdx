...
Unknown keyword arguments raise `TypeError` at decoration time. This includes
misspelled parameters and the removed `provider` argument. Select the provider
through the model ID instead, for example `"openai/gpt-4o-mini"`.
