...
In the [Prompts](/docs/prompts) guide, we talked about how the `@llm.prompt` decorator streamlines writing LLM powered functions. 
...
`@llm.call` does this via `llm.use_model()`, which can be used to manually similar behavior. See [Models](/docs/models).
</Note>
...
| `temperature`, `max_tokens`, etc. | Model parameters. See [Models](/docs/models). |
...
- [Thinking](/docs/thinking) — Use extended reasoning capabilities
