...
When any tool is async, the call must be async too, and you use `await` with `execute_tools()` and `resume()`. See the [Async](/docs/async) guide for more details.
...
- [Structured Output](/docs/structured-output) — Parse responses into typed objects
- [Streaming](/docs/streaming) — Stream responses with tool calls
- [Context](/docs/context) — Inject dependencies into tools
- [Agents](/docs/agents) — Build autonomous agents with tools
