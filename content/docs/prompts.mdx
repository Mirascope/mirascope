...
When working programmatically with LLMs, we often want reusable functions that encapsulate constructing messages and getting an LLM response. Using just what we've already learned about [Messages](/docs/messages) and [Models](/docs/models), we might write code like this:
...
| --- | --- |
| `tools` | List of tools the LLM can call. See [Tools](/docs/tools). |
| `format` | Response format for structured output. See [Structured Output](/docs/structured-output). |
...
- [Calls](/docs/calls) — Bundle a model with your prompt function
- [Tools](/docs/tools) — Let LLMs call functions
- [Structured Output](/docs/structured-output) — Parse responses into structured data
