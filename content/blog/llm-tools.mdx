...
In the example below, we preserve state by appending interactions to the `history` attribute of our chat, which is returned as part of the messages list. All this is passed as part of the prompt in future iterations of `run()`.
...
class Chat(BaseModel):
    ...
    def _step(self, question: str) -> list[llm.Message]:
        return [
            *self.history,
    ...
    def run(self) -> None:
        while True:
            ...
            self.history = list(response.messages)
...
**Note:** Different model providers have specific rules and limitations regarding their roles, so be sure to follow their role requirements when constructing message lists.
