...
![Mirascope homepage: LLM abstractions that aren't obstructions](/assets/blog/llm-tools/mirascope-homepage.webp)

‍[Mirascope](/docs) is a lightweight Python toolkit for developing LLM applications that gives you building blocks rather than a framework for setting up [prompt engineering workflows](/blog/prompt-engineering-best-practices). We purposefully designed Mirascope with simplicity and reliability in mind, and developing with Mirascope feels like you're simply writing the Python code you already know.
...
messages = recommend_music_prompt.messages(["short", "long"], ["rock", "jazz"])
print(messages)

# > [UserMessage(content='Can you recommend some music based on these duration and genre pairs?\nDuration: short, Genre: rock\nDuration: short, Genre: jazz\nDuration: long, Genre: rock\nDuration: long, Genre: jazz')]
...
Although `recommend_music_prompt` in the example above treats the prompt as a single user message for simplicity, you can add more context by incorporating roles using `llm.messages.system()`, `llm.messages.user()`, etc. to generate more relevant responses. Since Mirascope prompt templates are just Python functions, you can write them to fit any specific use case needed for your application.
...
We believe that all components that can affect the quality of a model call to an LLM - like prompt formatting, arguments, original responses from your model provider, and more, [should all live together in one place](/blog/engineers-should-handle-prompting-llms). This gives you a clear view of your call structure and allows for easier customization, leading to more efficient debugging and refinement.

Mirascope colocates calls and makes these the central organizing unit of the prompt, including [versioning](/blog/prompt-versioning) and testing. This ensures your code is clean and easily maintainable.
...
@llm.call("openai/gpt-4o", temperature=0.5)
def recommend_music(durations: list[str], genres: list[str]):
    durations_x_genres = "\n".join(
        f"Duration: {duration}, Genre: {genre}"
        for duration in durations
        for genre in genres
...
In the example below, we preserve state using `response.resume()`, which automatically maintains the full conversation history internally. Each call to `resume()` includes all previous messages, making it easy to build multi-turn conversations.
...
def chat():
    response = model.call(input("(User): "))
    ...
    while True:
        ...
        if question == "exit":
            break
        response = response.resume(question)
        print(f"(Assistant): {response.text()}")
...
For more complex agents that require additional state beyond conversation history (like tracking tool calls, maintaining context, etc.), you can wrap this pattern in a class and add any additional state management you need.

#### Streamlined Function Calling (Tools)

Mirascope lets you extend model capabilities [by adding tools](/docs/learn/llm/tools) (i.e., function calling) to your workflows. Tools allow LLMs to access external information, perform calculations, run code, and more, and are straightforward to set up with Mirascope's pythonic conventions.
...
@llm.call("openai/gpt-4o", tools=[get_stock_price])
def check_stock_price(stock: str):
    return f"What's the stock price for {stock}?"
...
A call response using Mirascope gives you a wrapper around the original response from the model, giving you access to the `tool_calls` property and `execute_tools()` method. This way, you can easily invoke the function and manage its arguments.

Our code uses Google-style Python docstrings, but we support other styles too, including ReST, Numpydoc, and Epydoc-style docstrings. For more code samples and use cases, please refer to our documentation on [tools (function calling)](/docs/learn/llm/tools).
...
To use Mirascope extraction, define a schema with a Pydantic `BaseModel` and set `format` to the schema in the decorator.

In the example below, `TaskDetails` defines the structure of the data with fields for `due_date`, and `priority`. By setting `format=TaskDetails`, we can call `.parse()` on the response to get an instance of `TaskDetails` with the correct details inside.
...
Mirascope currently supports [extracting built-in types](/docs/learn/llm/structured-output#built-in-types) including: `str`, `int`, `float`, `bool`, `list`, `set`, `tuple`, and `Enum`.

### LangChain

![LangChain homepage: Applications that can reason. Powered by LangChain.](/assets/blog/llm-tools/langchain_homepage.webp)
...
With LangChain, you can easily connect and query LLMs from your code through its APIs. The framework provides [predefined templates](/blog/langchain-prompt-template) and utilities for processing different data formats (e.g., PDF, HTML, and CSV) to prepare these for LLM consumption. Its chaining and agent features allow you to combine transformer language models with other components like retrieval systems and planners to build more intelligent and sophisticated applications.

‍[LangChain](/blog/llamaindex-vs-langchain) also integrates with platforms like Microsoft Azure, GCP, and more, achieving impressive benchmarks in performance and efficiency.

For more information, you can read about LangChain on its website, documentation site, and GitHub page.

### Helicone

![Helicone homepage: LLM-Observability for Developers](/assets/blog/llm-tools/helicone_homepage.webp)
...
![Ollama homepage: Get up and running with large language models.](/assets/blog/llm-tools/ollama_homepage.webp)
...
![vLLM in GitHub](/assets/blog/llm-tools/vllm_github_page.webp)
...
![Open LLM in GitHub](/assets/blog/llm-tools/open_llm_github_page.webp)
...
![Rasa homepage: Let's Build Conversational AI Experiences](/assets/blog/llm-tools/rasa_homepage.webp)
...
![Botkit in GitHub](/assets/blog/llm-tools/botkit_github_page.webp)
...
![Pinecone homepage: Build knowledgeable AI](/assets/blog/llm-tools/pinecone_homepage.webp)
...
![Milvus homepage: The High-Performance Vector Database Built for Scale](/assets/blog/llm-tools/milvus_homepage.webp)
...
![crewAI homepage: AI Agents for real use cases](/assets/blog/llm-tools/crewai_homepage.webp)
...
![AutoGen in GitHub](/assets/blog/llm-tools/autogen_github_page.webp)
