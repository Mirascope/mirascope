...
def tree_of_thought(task: str):
    return [
        llm.messages.system("When given a task, consider at least three different approaches first, outlining the pros and cons of each approach. Then choose the best one."),
        llm.messages.user(task),
    ]
...
"""
Designing a new public park in a city with limited space involves careful consideration of various factors such as community needs, aesthetics, and environmental impact. Here are three different approaches:

### Approach 1: Vertical Garden Park
**Description**: Transform a small urban area into a vertical park, utilizing multi-story planter walls with greenery, walking paths, seating areas, and small gathering spaces.

**Pros**:
- **Maximizes Limited Space**: Vertical space allows for more greenery and recreational areas in a compact footprint.
- **Aesthetic Appeal**: Creates a visually striking environment, attracting visitors and enhancing community pride.
- **Air Quality Improvement**: Plants improve air quality and provide shade, contributing to a healthier urban ecosystem.

**Cons**:
- **Higher Costs**: Construction and maintenance of vertical gardens can be more expensive than traditional parks.
- **Limited Use**: May not provide sufficient space for larger recreational activities or gatherings.
- **Complex Maintenance**: Requires specialized knowledge for plant care and structural integrity.

### Approach 2: Multi-Functional Green Space
**Description**: Create a flat, multi-use park that incorporates diverse functions, such as a playground, small sports courts, picnic areas, and native landscaping for biodiversity.

**Pros**:
- **Community Engagement**: Offers varied activities catering to different age groups and interests, promoting social interaction.
- **Cost-Effective**: Easier and potentially less expensive to construct and maintain compared to elaborate structures.
- **Versatile Use**: Space can be adapted for events, markets, and community gatherings.

**Cons**:
- **Space Limitations**: Balancing different activities may lead to conflicts (e.g., noise levels).
- **Less Greenery**: Might result in less overall green space compared to a vertical garden design.
- **Potential for Overcrowding**: High usage can lead to wear and tear or a feeling of congestion.

### Approach 3: Urban Pocket Park
**Description**: Establish a small, dedicated pocket park that includes essential features like benches, native plants, a small water feature, fitness stations, and pathways for walking or jogging.

**Pros**:
- **Easily Accessible**: Small parks can be located near busy areas, providing quick access for city dwellers.
- **Cost-Effective**: Generally lower construction and maintenance costs compared to larger parks.
- **Community Revitalization**: Can enhance surrounding neighborhoods, creating a more inviting atmosphere.

**Cons**:
- **Limited Activities**: Smaller space restricts recreational opportunities and amenities.
- **Potential Isolation**: If not well-planned, it could feel underutilized or safety concerns may arise.
- **Seasonal Usage**: May be more heavily used in certain seasons or weather conditions.

### Best Approach: Multi-Functional Green Space
After evaluating the options, the **Multi-Functional Green Space** approach is the most suitable. It provides a balance of accessibility, community engagement, and versatility. The potential for diverse uses appeals to a wider demographic, fostering a sense of community and encouraging outdoor activity. While it may face challenges with space and possible overcrowding, careful planning and design can mitigate these issues and create a vibrant community hub within a limited area.
"""
```
...
@llm.call("openai/gpt-4o-mini", format=list[Thought])
def think(task: str):
    return [
        llm.messages.system("When given a task, think through at least three different approaches. Outline the pros and cons of each approach."),
        llm.messages.user(task),
    ]

@llm.call("openai/gpt-4o-mini")
def tree_of_thought(task: str):
    thoughts_response = think(task)
    thoughts_list = thoughts_response.parse()
    thoughts_text = "\n\n".join([
        f"Approach: {thought.approach}\nPros: {thought.pros}\nCons: {thought.cons}"
        for thought in thoughts_list
    ])
    return f"""Based on the following thoughts on how to solve the task, select the best approach.
...
# First, generate and display the structured thoughts
thoughts_response = think(task)
thoughts = thoughts_response.parse()
print("Thoughts:")
for thought in thoughts:
    print(f"Approach: {thought.approach}")
    print(f"Pros: {thought.pros}")
    print(f"Cons: {thought.cons}")
    print()

# Then, get the final evaluation using tree_of_thought
response = tree_of_thought(task)
...
"""
Thoughts:
Approach: Vertical Garden Park
Pros: ['Maximizes limited space by utilizing vertical structures.', 'Enhances aesthetic appeal with green walls and hanging gardens.', 'Provides a unique experience for visitors, encouraging exploration.']
Cons: ['Higher initial construction costs for vertical systems.', 'Requires ongoing maintenance for plants and structures.', 'Potential challenges with sunlight exposure for all plants.']

Approach: Multi-Use Space
Pros: ['Accommodates various activities (playgrounds, sports, community events) in a compact area.', 'Encourages community interaction and diverse usage.', 'Easily adaptable to different seasons or events.']
Cons: ['Potential for overcrowding during peak times.', 'Conflicts between different user groups (e.g., quiet vs. active).', 'Requires careful planning to ensure all areas are well-designed for safety.']

Approach: Pocket Park with Green Amenities
Pros: ['Creates a resting spot amidst urban hustle, offering a mini escape.', 'Encourages biodiversity with native plants and habitat creation.', 'Low impact on existing structures and surroundings.']
Cons: ['Limited space may restrict amenities and activities.', 'Could be underutilized if not designed to attract visitors.', 'Maintaining greenery in a confined area can be challenging.']

Final Evaluation:
The best approach is the **Multi-Functional Urban Space**.

Evaluation:
This approach effectively serves various community needs, encourages engagement, and allows adaptability to seasonal activities, which fosters a vibrant urban environment. While it has challenges like potential overcrowding and complex maintenance, the benefits of community interaction and diverse uses outweigh these concerns. The other options either face significant construction and maintenance challenges or do not provide enough space for community activities, limiting their overall impact.
"""
```
...
Mirascope's `@llm.call` decorator (see the [call documentation](/docs/learn/llm/calls)) serves as a kind of [template](/blog/langchain-prompt-template) to formulate model-agnostic prompts without needing to make major changes to the prompt structure or content.
...
@llm.call("openai/gpt-4o-mini", temperature=0.9)
def report_driving_time(city1: str, city2: str):
    return f"Provide the estimated driving time from {city1} to {city2}"

response = report_driving_time(city1="Los Angeles", city2="San Francisco")
print(response.text())  # prints the string content of the call
```
...
For instance, `response` below is assigned the output of an OpenAI call that returns a structured output. When you hover the cursor over the `response` object in your editor, it’s identified as being a `Book`.

![Structured Output example](/assets/blog/llm-prompt/mirascope-type-safety.webp)
...
It's helpful to use a local working directory for this. Mirascope offers a dedicated prompt management framework called [Mirascope Cloud](/docs/learn/ops) that versions and traces every prompt and LLM call automatically.

## Use Mirascope for Effective Prompting

Mirascope is a lightweight Python toolkit that's premised on software engineering best practices. Our library offers tools for reducing boilerplate when writing LLM functions, and Mirascope Cloud offers prompt management and automatic versioning and tracing as a symbiotic partner library.
...
Want to learn more? You can find more Mirascope code samples both on our [docs](/docs) and the [GitHub repository](https://github.com/mirascope/mirascope).
