...
from mirascope import llm
from mirascope.llm import DynamicConfig, MessageParam, Messages


def get_relevant_messages(
    query: str, history: list[MessageParam]
) -> list[MessageParam]:
...
def correct_factual_errors(
    ...
    return {
        "messages": [
            llm.Messages.User(f"""{previous_errors}
...
class Librarian(BaseModel):
    ...
    def _call(self, query: str) -> DynamicConfig:
        messages = [
...
In the code above, `Librarian` maintains conversation history using a list of `MessageParam` objects.
