...
* Provide pre-built modules for common tasks like data preprocessing, model [fine-tuning](/blog/prompt-engineering-vs-fine-tuning/), and response generation.
...
But [modern frameworks](/blog/llamaindex-vs-langchain/) (like LangChain) impose their own unique abstractions, requiring you to do things their way. This not only feels limiting but also makes development and maintenance harder than it needs to be.
...
These frameworks offer [tools](/blog/llm-tools/) and libraries that simplify and scale the process of building, testing, and deploying LLM-based applications.

Below are some of the most effective frameworks that facilitate this process.

### Mirascope

![Mirascope Homepage](/assets/blog/llm-frameworks/mirascope-homepage.webp)
...
This is in contrast to LangChain, whose elaborate abstractions like [runnables](/blog/langchain-runnables) (consisting of Python callables wrapped in LangChain Expression Language, or LCEL) obscure their underlying logic.
...
def overview(subject: str):
    ...
    return f"""Use the following pieces of information to provide a concise overview.
If the details are insufficient, state that clearly without making assumptions.
Keep your response to three sentences for conciseness.
End with "hope this provides clarity!" to conclude your response.
...
response = overview("Describe machine learning")
print(response.text())
```
...
The above snippet shows how you can build a RAG pipeline using [LangChain](/blog/langchain-sucks/)'s `retriever` object for retrieving the relevant documents from the Chroma vector store but use Mirascope for writing the `overview` function instead of a LangChain chain for formatting your prompt and making the LLM call.

#### A Convenient Approach to LLM Calling

Mirascope's [call decorator](/docs/learn/llm/calls) (e.g., `@llm.call()`) converts regular Python functions into [prompts](/blog/advanced-prompt-engineering/) by turning the function signature and return statement into an API request.
...
It also [colocates your calls with your prompts](/blog/engineers-should-handle-prompting-llms) to avoid separating them in your codebase, thus improving your code’s readability and maintainability.
...
You can reuse the same [prompt logic](/blog/llm-prompt/) across different model providers.

While Mirascope's call decorators make your life easier when it comes to integrating various LLM providers, the library also provides functionality for dynamically configuring LLM behavior at runtime.

**Runtime configuration** with `llm.model()` lets you override model parameters like temperature, switch providers, or adjust settings on the fly - all in a provider-agnostic way.

It's useful in scenarios where you need to change certain settings based on runtime conditions. For example, you may want to implement chatbots that take the message history as an input argument but only insert certain messages or the most recent messages into the prompt.

The code sample below shows an example of inserting only the most relevant messages and adjusting temperature dynamically using the `llm.model()` context manager:
...
def get_relevant_messages(
    query: str, history: list[llm.Message]
) -> list[llm.Message]:
...
@llm.call("mistral/mistral-small-latest")
def chatbot(query: str, history: list[llm.Message]):
    return [
        llm.messages.system("You are a helpful chatbot"),
...
temperature = 0.25 if creativity == "low" else 0.75
with llm.model("mistral/mistral-small-latest", temperature=temperature):
    response = chatbot("What is the best way to learn programming?", [...])
    print(response.text())
```

Runtime configuration is also useful in Mirascope's call chaining, where the output of one model's response is injected into the prompt of another.

Since `@llm.call` decorated functions are just regular Python, you can easily chain calls together by using one call's output as input to another. The `response.messages` property includes the full conversation history, making it easy to continue conversations or build complex workflows.

For example, a customer support chatbot might first need to understand why a customer is contacting it in the first place before finally responding, so the chain could first run another sub-call to identify the customer's issue, then inject that result into the final response.
...
On the contrary, Mirascope offers its built-in **`format`** parameter with the `.parse()` method, allowing you to easily define, structure, and validate outputs using Pydantic's `BaseModel`.
...
![OpenAIStream Type Hint](/assets/blog/llm-frameworks/openai-stream-type-hint.webp)

It also shows autosuggestions:

![Mirascope Prompt Template Autocomplete Suggestion](/assets/blog/llm-frameworks/prompt-template-autocomplete.webp)  
You implement structured outputs by:

1. Extending `BaseModel` to represent the desired structure of the LLM's output.
2. Setting the `format` argument in a `@llm.call` decorator to the class you just defined previously, then calling `.parse()` on the response.
...
def extract_task(text: str) -> str:
    return f"Extract the task details from this text: {text}"

response = extract_task(text="Finish the report by Friday, it's high priority")
task = response.parse()
print(task.due_date)  # Output: Friday
print(task.priority)  # Output: high
```

When `extract_task` is called, Mirascope uses the `TaskDetails` model to parse the LLM's output via `.parse()`, making `due_date` and `priority` accessible as attributes of the `task` object.

While response models ensure that your [outputs](/blog/langchain-structured-output/) are well structured and validated, handling unexpected errors is equally important to making your code robust.
...
One effective way of using Tenacity is to incorporate previous validation errors into subsequent [prompts](/blog/prompt-engineering-examples/) to guide the model to refine its responses using context-driven adjustments.
...
@llm.call("anthropic/claude-3-5-sonnet-20240620", format=FactCheck, json_mode=True)
def check_for_factual_errors(text: str) -> str:
    return f"Does the following text contain any factual errors? {text}"

def validate_factual_accuracy(t: str) -> str:
    response = check_for_factual_errors(t)
    if response.parse().has_errors:
        raise ValueError("Text still contains factual errors")
    return t

class FactCorrection(BaseModel):
    text: Annotated[
        str,
        AfterValidator(validate_factual_accuracy),
...
@llm.call("anthropic/claude-3-5-sonnet-20240620", format=FactCorrection, json_mode=True)
...
try:
    ...
    response = correct_factual_errors(text)
...
A `FactCheck` class is first defined to validate whether text contains factual errors, using Mirascope's `@llm.call` decorator to ensure responses fit the expected structure.
...
The `previous_errors` field includes past validation errors as contextual information in [subsequent prompts](/blog/prompt-engineering-best-practices/), allowing the model to leverage insights from prior outputs to refine its responses - similar to how few-shot learning uses examples to guide the model.

### LangChain

![LangChain Homepage](/assets/blog/llm-frameworks/langchain-homepage.webp)

[LangChain](https://www.langchain.com/) is an open-source framework built for developing [applications](/blog/llm-applications/) that rely on language models. Available as both Python and JavaScript libraries, it offers an environment where you can orchestrate LLM-based pipelines with various data sources and workflows.

As the name suggests, LangChain centers around [chaining steps](/blog/prompt-chaining/) needed to sequence multiple calls to an LLM, allowing for complex interactions and iterative processing that build upon previous responses.
...
![LlamaIndex Homepage](/assets/blog/llm-frameworks/llama-index-homepage.webp)
...
![Haystack Homepage](/assets/blog/llm-frameworks/haystack-homepage.webp)

[Haystack](https://haystack.deepset.ai/) is an end-to-end framework for building NLP applications using large language models. With Haystack, you can create question-answering systems, conversational agents, and other applications by linking LLMs, vector databases, and related components through [configurable pipelines](/blog/llm-pipeline/).
...
![Ask Marvin Homepage](/assets/blog/llm-frameworks/ask-marvin-homepage.webp)
...
Workflow [orchestration frameworks](/blog/llm-orchestration/) manage and coordinate interactions to improve the performance and effectiveness of LLM-powered applications.

### Orkes

![Orkes Homepage](/assets/blog/llm-frameworks/orkes-homepage.webp)
...
![ControlFlow AI Homepage](/assets/blog/llm-frameworks/control-flow-homepage.webp)
...
ControlFlow AI integrates easily with existing tech stacks and is compatible with most [LangChain](/blog/langchain-alternatives/)-supported LLMs. By leveraging Prefect 3.0, its orchestration and observability features, the framework improves AI capabilities with Prefect’s reliable orchestration and observability.
...
def available_books() -> str:
    ...
    return json.dumps([book.model_dump() for book in library])


class Librarian(BaseModel):
    history: list[llm.Message] = []

    @llm.call("openai/gpt-4o-mini", tools=[available_books])
    def _call(self, query: str):
        return [
            llm.messages.system("You are a librarian"),
            *self.history,
            llm.messages.user(query),
        ]
    ...
    def _step(self, query: str) -> str:
        if query:
            self.history.append(llm.messages.user(query))
        ...
        if tool_calls := response.tool_calls:
            for tool_call in tool_calls:
                ...
                tool_outputs.append(response.toolkit.execute(tool_call))
            self.history.append(llm.messages.user(tool_outputs))
            return self._step("")
        else:
...
In the code above, `Librarian` maintains conversation history using a list of `llm.Message` objects.

The `_call` method constructs a sequence of messages for the API call `@llm.call("openai/gpt-4o-mini")`, starting with a system message ("You are a librarian"), followed by the stored conversation history and finally by the user's query.

We structure the response using Mirascope's `Messages`, which ensures consistent and provider-agnostic message formatting.

We also give the agent access to the `available_books` tool so that it knows which books are available in the library to recommend.
...
For examples of complex, real-world agents, we recommend checking out our [agent tutorials](/docs/tutorials/agents/web-search-agent).

### Microsoft Semantic Kernel

![Microsoft Semantic Kernel GitHub](/assets/blog/llm-frameworks/msft-semantic-kernel-github.webp)
...
![AutoGen Homepage](/assets/blog/llm-frameworks/autogen-homepage.webp)
...
Want to learn more? You can find more Mirascope code samples both on our [documentation site](/docs) and on our [GitHub repository](https://github.com/mirascope/mirascope).
