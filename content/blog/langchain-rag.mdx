...
Take [runnables](/blog/langchain-runnables) for instance, which LangChain relies on for chaining together steps to handle prompt flows:
...
**Mirascope avoids the rigid, all-or-nothing approach you find in the [big frameworks](/blog/llamaindex-vs-langchain)**, and instead offers a flexible architecture that lets you select only the modules you need, while giving you full control over [prompting](/blog/prompt-engineering-examples), context management, and LLM interactions.
...
![RAG Pipeline Diagram](/assets/blog/langchain-rag/rag-pipeline-diagram.webp)
...
The first thing is to load your data from various sources, like PDFs, websites, or text documents, and prepare these for later stages [in the pipeline](/blog/llm-pipeline).
...
Keep in mind, though, we don’t have to use vectors - we could insert a [knowledge graph](/blog/how-to-build-a-knowledge-graph/) (or use any other appropriate structure like a document store) - it’s just that vectors are efficient under certain circumstances, like when querying large volumes of unstructured data.
...
Once LangChain has identified and retrieved the relevant information, **it combines the fetched chunks with the original user query to create a [context-enriched prompt](/blog/llm-prompt)** to elicit a targeted answer from the LLM.

### Challenges in Building a RAG System with LangChain

Although [LangChain](/blog/langchain-sucks) offers tools for simplifying the development of RAG applications, the entire process isn’t without its pains.
...
But **LangChain’s complex architecture doesn’t make things easier**. The [framework](/blog/llm-tools) requires multiple dependencies and installations of various components (i.e., core, hub, and community), while many report its documentation is outdated.
...
An example of this is LangChain’s familiar chaining structure, or the sequence [orchestrating](/blog/llm-orchestration) retrieval, formatting, prompting, and response generation in a RAG application (e.g., `rag_chain`):
...
* [Prompting](/blog/prompt-engineering-tools) (via `custom_rag_prompt`)  
* LLM response generation (via `llm`)  
* [Output parsing](/blog/langchain-structured-output) (via `StrOutputParser()`)

On the surface, the flow of inputs and outputs between these components seems logical, since the pipe moderators provide an intuitive way to [chain these together](/blog/prompt-chaining).
...
Now that we know how RAG works on a theoretical level, here’s a simple tutorial showing how to build a simple RAG application using [LangChain](/blog/langchain-alternatives), Chroma, and Mirascope.
...
* **`OpenAIEmbeddings`** is an embedding model for encoding chunks into vectors, allowing [LLM-based applications](/blog/llm-applications) to understand the relationships between different words.  
...
After using LangChain to set up document ingestion and storage, we then search the vector store for relevant context based on the user's input query, and use Mirascope to format and [optimize the prompt](/blog/prompt-engineering-best-practices) that integrates both the retrieved context and the query.
...
def query_documents(query: str, vector_store: Chroma) -> str:
...
response = query_documents(query=query, vector_store=vector_store)
print(response.text())
...
Next, **`context`** returns the page content of the relevant chunks, which is included in the prompt template string.

This **`context`**, along with the original **`query`**, is passed to the language model via Mirascope's **`@llm.call`** decorator and generates a response based on the provided information.

Our call decorator allows you to transform [prompt templates](/blog/langchain-prompt-template) written in Python into API calls to the LLM using minimal boilerplate - making Mirascope model agnostic - while providing type safety and consistency across providers.
...
Simplify your RAG workflows and dispense with the complexities of the [big frameworks](/blog/prompt-flow-vs-langchain) by using Mirascope’s lightweight toolkit, whose modules easily slot into existing workflows and give you control over [prompting](/blog/engineers-should-handle-prompting-llms) and LLM interactions.

You can find Mirascope code samples and tutorials both on our [documentation site](/docs) and on our [GitHub page](https://github.com/mirascope/mirascope).
