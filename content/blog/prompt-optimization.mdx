...
In this guide, we'll walk you through 10 basic and advanced ways for optimizing prompts, and illustrate some of these using examples from our own framework for managing and improving LLM prompts, Mirascope Cloud.
...
Prompts include call parameters like temperature, model choice, and even surrounding logic, making them deeply integrated with the [LLM tools](/blog/llm-tools) that shape their behavior. **That's why Mirascope Cloud advocates wrapping each prompt in a Python function, complete with typed arguments and structured return values**, since such functions are modular, clear, and easy to reason about.

By adding the `@ops.version` decorator to a function, you mark it as a versioned, non-deterministic unit:
...
For instance, in Mirascope Cloud every function decorated with `@ops.trace` is automatically traced and logged. This includes not only the prompt and inputs, but also the outputs, execution time, token counts, and other metadata.
...
The code below shows how Mirascope Cloud traces nested function calls, where both the `parent()` and `child()` functions are decorated with `@ops.trace`, allowing their inputs, outputs, and execution metadata to be automatically captured and visualized as linked spans in the trace view:
...
The Mirascope Cloud playground displays the results of the traced execution, capturing both the `parent()` and nested `child()` calls as structured spans, giving you full visibility into their inputs, outputs, and execution metadata, so you can inspect how and when each function ran:

![Mirascope Cloud Nested Traces](/assets/blog/_lilypad/lilypad-nested-traces.webp)
...
In Mirascope Cloud, for example, every time an `@ops.version`-decorated function is executed, it's tied to the exact version of the function that ran. You can rerun any past version using `.version(n)`:

```python-snippet-skip
response = answer_question.version(7)("Who painted the Mona Lisa?")
```

Mirascope Cloud keeps track of versions as well:

![Lilypad Answer Question v7 Trace](/assets/blog/_lilypad/lilypad-trace.webp)

You can also compare different versions side by side, showing all relevant metrics:

![Lilypad Compare Answer Question Versions](/assets/blog/_lilypad/lilypad-compare-versions.webp)
...
Under the hood, Mirascope Cloud automatically versions the entire function closure with every change, including any user-defined functions or classes outside the main prompt function (as long as they're within scope). Also, identical runs aren't duplicated and only meaningful changes are tracked.
...
By adding the `@ops.trace` decorator to any function, you effectively add observability to LLM calls and expose their characteristics and outputs in the playground.
...
This separation of concerns, **with engineers owning the function scaffolding and SMEs owning the iterations**, lets teams move faster without stepping on each other's toes. When ready, a versioned, effective prompt can be hot-swapped into production by referencing it directly (e.g., `.version(n)`) all without breaking the build or risking untested changes.
...
In Mirascope Cloud, for instance, tracing and logging over brittle test cases is emphasized, so instead of asking, "Did this return the exact expected answer?" you should be asking, "Is this version of the prompt more useful, more consistent, or more relevant over time?"

Mirascope Cloud supports evaluation metrics through annotations, which let you manually label specific runs with pass or fail judgments and freeform reasoning to improve prompt performance.
...
For in-code workflows, Mirascope Cloud provides a `trace.annotate()` method, which lets you tag and evaluate outputs directly from within [LLM applications](/blog/llm-applications), without modifying function signatures or return values. This makes it simple to fold prompt evaluation into CI pipelines, scripted tests, or automated reporting.
...
trace = answer_question.wrapped("Who painted the Mona Lisa?")  # [!code highlight]
print(trace.result.text())  # original response
# > Leonardo da Vinci painted the Mona Lisa.
...
Prompting doesn't end at generation: it begins there. Mirascope Cloud traces every AI prompt, captures every version, and helps you evaluate responses over time. That means a better and more effective prompt, fewer regressions, and full transparency into your [LLM integration](/blog/llm-integration).

Want to see it in action? Explore our [docs](/docs/learn/ops) or check out our [GitHub](https://github.com/mirascope/mirascope). Mirascope Cloud offers first-class support for [Mirascope](/docs), our lightweight toolkit for building [LLM agents](/blog/llm-agents).
