...
[Mirascope Cloud](/docs/learn/ops) is an open source context engineering framework that treats LLM interactions like an optimization problem, recognizing that reliable outputs depend on the entire context, not just the prompt.
...
Mirascope Cloud also traces every LLM call, capturing inputs, outputs, costs, and latency to give developers complete visibility in analyzing and debugging model outputs.

As a lightweight open source tool, Mirascope Cloud integrates with existing workflows and libraries (like LangChain, Mirascope, or direct provider SDKs), making it framework-agnostic. It can also be self-hosted.

Mirascope Cloud supports popular models like OpenAI, Google, AWS Bedrock, Anthropic, Azure, and many others.

Below, we highlight how Mirascope Cloud helps you manage context in building LLM applications.

#### Structuring and Managing Context

To ensure consistency across iterations, Mirascope Cloud encourages developers to organize their code and prompts within Python functions that encapsulate LLM calls.
...
For example, below we define a function `answer_question` and decorate it with `@ops.version` to set up tracing with every run, and automatically version everything within the function's closure whenever a change is made:
...
Both Mirascope Cloud and Mirascope (for which Mirascope Cloud gives first-class support) provide out-of-the box pythonic abstractions for managing the context fed into the prompt.

For example, in the code above, [Mirascope's `@llm.call` decorator](/docs/learn/llm/calls) turns the prompt function into a call with minimal boilerplate code.

`@llm.call` provides a unified interface for working with model providers like OpenAI, Grok, Google (Gemini/Vertex), Anthropic, and many others, and you can change the provider by changing the model string (e.g., `"anthropic/claude-sonnet-4-20250514"`).

This decorator also provides an interface for [tool calling](/docs/learn/llm/tools), [structured outputs](/docs/learn/llm/structured-output), Pydantic-based input validation, [prompt chaining](/blog/prompt-chaining), type hints (integrated into your IDE), and others.

You can work with prompts and their associated code within the Mirascope Cloud UI, a no-code environment that lets non-technical users test, edit, and evaluate prompts while keeping them tied to the exact versioned code that runs in production.

![Mirascope Cloud Trace GenAI Data](/assets/blog/_lilypad/lilypad-trace-genai-data.webp)

Mirascope Cloud uses the [OpenTelemetry Gen AI spec](https://opentelemetry.io/) to instrument calls and their surrounding code, capturing metadata like costs, token usage, latency, messages, warnings, and others.

Mirascope Cloud traces LLM calls not only at the level of the API to tell you what the model call did as LLM call spans, but also at the function level, capturing how your code shaped the call through inputs, logic, and outputs.

The Mirascope Cloud UI also shows the output of every call, as well as any changes made to the prompt or its context:

![Mirascope Cloud Trace](/assets/blog/_lilypad/lilypad-trace.webp)
...
Mirascope Cloud's playground is a [prompt management tool](/blog/prompt-management-tool) allowing you to create, modify, test, and evaluate prompts without needing to alter the underlying code.
...
Every time a user runs an [LLM prompt](/blog/llm-prompt) in the UI, Mirascope Cloud automatically versions and traces the prompt against the underlying function.
...
Mirascope Cloud takes a different approach by versioning prompts as part of the codebase itself, ensuring that what runs in the playground is the same type-safe code developers run downstream in production, eliminating the brittleness of separating prompts from the rest of the system.
...
To make use of these datasets, Mirascope Cloud provides lightweight evaluation tools that turn raw runs into actionable feedback.
...
Mirascope Cloud also offers features allowing teams to collaborate on evaluations and discuss results, as well as to assign tasks to other users.
...
![Llama Index Diagram](/assets/blog/_shared/llama-index-diagram.webp)
...
![PromptLayer Diagram](/assets/blog/_shared/prompt-layer-diagram.webp)
...
Mirascope Cloud helps you version, trace, and evaluate every factor that shapes an LLM's output, allowing you to reproduce results, debug faster, and ensure your applications behave reliably as they grow in complexity.

Want to learn more? Check out Mirascope Cloud code samples on our [documentation site](/docs/learn/ops) and [GitHub](https://github.com/mirascope/mirascope). Mirascope Cloud offers first-class support for [Mirascope](/docs), our lightweight toolkit for building AI agents.
