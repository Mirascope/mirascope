...
Unlike [LLM evaluation](/blog/llm-evaluation/), which assesses a model's overall performance across various tasks (its general strength), **prompt evaluation zooms in on the prompt itself** - judging how well a particular prompt is structured to produce your desired output - be it in customer support, content generation, or other use cases.
...
In this article, we show you different ways to evaluate prompts and share practical techniques and insights based on real-world applications. Along the way, we'll also examine how [Mirascope Cloud](/docs/learn/ops) helps structure this evaluation process, making it easier to iterate, compare, and optimize prompts at scale.
...
[Mirascope Cloud](/docs/learn/ops) allows you to automatically version, trace, and manage prompts (functions) by simply adding the Python decorator `@ops.trace` to your LLM functions.
...
Mirascope Cloud also allows easy collaboration between engineers and domain experts via a managed user interface, where non-technical users can tweak and evaluate prompts without altering (or even necessarily seeing) the underlying code, simplifying the process for non-technical users without needing to change engineering workflows:

![Mirascope Cloud Playground](/assets/blog/prompt-evaluation/lilypad_editor_screenshot.webp)

Mirascope Cloud's design provides a structured approach to optimizing prompts from the get-go, allowing for controlled experiments and efficient annotation of LLM outputs.
...
Cosine similarity is extensively used in RAG (e.g., [LangChain RAG](/blog/langchain-rag/)) applications to retrieve content from a vector store that’s semantically relevant to a user’s query, before sending this as additional context (together with the query) to the language model.

(Read also about how we build a [RAG LLM example](/blog/rag-llm-example/).)
...
If you need to explore a wide variety of scenarios, [synthetic data generation](/blog/synthetic-data-generation/) can help seed your tests with realistic but artificially created user inputs. This ensures you have sufficient coverage in early tests before actual user traffic ramps up.
...
This is especially useful in [LLM applications](/blog/llm-applications/) requiring high engagement or trust, such as customer support or education, where user satisfaction is directly tied to the success of the system.  
...
(For more information and a tutorial on chatbots, see our article on [How to make a chatbot](/blog/how-to-make-a-chatbot/).)
...
[Mirascope Cloud](/docs/learn/ops) automatically versions and traces all prompt changes to support experiments and collaboration, allowing you to identify what worked and what didn't. This includes:

* Automating experimentation and detecting regressions, such as A/B testing of prompt variations
* Historically comparing the effectiveness of different prompts, via Mirascope Cloud's traceability
* Providing context-aware evaluations entailing factors like token usage, latency, and model parameters
* Letting you run custom evaluation metrics
...
First, we define a Pydantic model (`CandidateEvaluation`) to structure the evaluation output. Then, using Mirascope's `@llm.call` decorator, we create an asynchronous function `evaluate_candidate_suitability` that encapsulates the entire prompt evaluation workflow.
...
@llm.call("anthropic/claude-sonnet-4-20250514", format=CandidateEvaluation, json_mode=True)
...
if __name__ == "__main__":
    ...
    response = asyncio.run(evaluate_candidate_suitability(role_req, candidate, recommendation))
...
When you run the `run(...)` function with these inputs, the system evaluates whether the LLM's recommendation aligns with the role requirements, taking into account factors like skills, experience, and soft skills:
...
result = asyncio.run(run(role_requirements, candidate_profile, llm_recommendation))
print(result)
```
...
You can find more Mirascope code samples both on our [documentation site](/docs) and our [GitHub page](https://github.com/mirascope/mirascope).
