...
We'll also show how [Mirascope Cloud](/docs/learn/ops), our open source prompt engineering framework, adds structure and traceability when working with unpredictable LLM outputs.
...
def summarize(content: str):
    return f"""Please provide a summary for the following text:
{content}"""
...
We'd use a simple decorator for the call and return a string from the function to define the prompt, keeping the implementation straightforward. 
...
Here's a quick example where `explain_scientist` first calls `recommend_scientist` and uses the result in the prompt:
...
def recommend_scientist(field: str):  # [!code highlight]
    return f"""Recommend a renowned scientist in the field of {field}.
Give me just the name."""

@llm.call("openai/gpt-4o")
def explain_scientist(field: str):  # [!code highlight]
    scientist_name = recommend_scientist(field).text()  # [!code highlight]
    return [
        llm.messages.system(f"""You are an expert historian of science.
Your task is to explain why the scientist "{scientist_name}" is highly regarded in the field of {field}."""),
        llm.messages.user(f'Explain why "{scientist_name}" is renowned in {field}.'),
    ]
...
Since we're using regular Python function calls, we can easily inspect intermediate results:

```python-snippet-skip
print(explanation.raw.model_dump_json(indent=2))
# {
...
In contrast, Mirascope implements type safety via its Pydantic-based [structured output](/docs/learn/llm/structured-output), enforcing what values functions return, and how LLM calls interact with those functions.
...
Hovering over `response` below shows it expects a response with a `Book` return type from `.parse()`:

![Mirascope Type Safety](/assets/blog/_shared/mirascope-type-safety.webp)
...
We've talked about LangSmith, LangChain's tool for tracing and observability. Now let's break down where it falls short and how Mirascope Cloud does it better.
...
Mirascope Cloud auto-versions everything: not just the prompt, but also the Python function that calls the LLM. That includes the model config (like temperature or model name), any helper functions, and all the logic inside.
...
With Mirascope Cloud's `@ops.version` decorator, you can wrap your LLM function and capture a full snapshot of everything that influenced that call. This also makes it easier to version and monitor API-based LLM calls, especially when integrating third-party APIs, internal tools, or other cloud services.

Any change to the function: prompt tweak, parameter change, helper edit, etc., creates a new version automatically when using the `@ops.version` decorator.
...
You see all this in Mirascope Cloud's playground, an easy-to-use interface that lets both devs and non-devs tweak prompts without touching the code or needing to get someone to redeploy every time they want to test a change.
...
Mirascope Cloud automatically creates a nested trace whenever the function is executed:
...
### Mirascope Cloud's Unified Evaluation Workflow
...
Mirascope Cloud's open source framework solves this by fully capturing everything: your function code, prompts, parameters, and helper logic. So even if you're chaining together multiple functions, you get a complete trace of every step. That makes it way easier to evaluate how each part contributed to the final result.

Mirascope Cloud also keeps things simple with a binary scoring system: pass or fail, plus a reason why. No fuzzy 1-to-5 scale. This makes feedback for [prompt evaluation](/blog/prompt-evaluation/) use cases clearer and more consistent, by determining what's "good enough," especially when reviewing tricky steps in longer workflows.

In the Mirascope Cloud playground you evaluate outputs by picking any output:

![Mirascope Cloud Answer Question Assign Annotation](/assets/blog/_lilypad/lilypad-answer-question-assign-annotation.webp)
...
Mirascope Cloud's unified [LLM evaluation](/blog/llm-evaluation/) workflow provides a better dev experience whether you're testing LLM apps built on an SDK or building apps like chatbots because it ties every output directly to the exact code, prompt, and settings that produced it. 
...
To get started with Mirascope Cloud, you can sign up for the [playground](https://cloud.mirascope.com/) using your GitHub credentials and instantly begin tracing and versioning your LLM functions with just a few lines of code.

## A Better Stack for LLM Development

Mirascope and Mirascope Cloud are built on a simple idea: **use what you need, skip what you don't**. Instead of locking you into a bloated framework, they give you flexible [LLM tools](/blog/llm-tools/) you can mix and match, from LangChain, LlamaIndex, LangGraph, or wherever else, to build exactly the [LLM integration](/blog/llm-integration/) and generative AI app you want.

When you use Mirascope Cloud alongside Mirascope, you get automatic versioning and tracing for every LLM function. That means it tracks not just your prompts, but also the code behind them, so you always know what ran and why.

Want to learn more? You can find more Mirascope Cloud code examples on both our [documentation site](/docs/learn/ops) and [GitHub](https://github.com/mirascope/mirascope). Mirascope Cloud offers first-class support for [Mirascope](https://github.com/mirascope/mirascope), our lightweight toolkit for building AI agents. 
