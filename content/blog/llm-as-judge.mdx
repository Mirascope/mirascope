...
1. Create a prompt containing the context, instructions, and [evaluation criteria](/blog/llm-evaluation/) to guide the LLM’s judgment.  
...
Responses are evaluated independently, without comparing them to others, and rely on predefined evaluation criteria like accuracy, clarity, relevance, and comprehensiveness, which should be outlined in the [prompt](/blog/llm-prompt/).
...
* [Prompting](/blog/advanced-prompt-engineering/) the model to provide explanations or critiques alongside its judgments to get insights into the LLM's reasoning process.  
...
* [Fine-tune](/blog/prompt-engineering-vs-fine-tuning/) judges on specific datasets to improve their accuracy.  
...
We use a Mirascope prompt decorator to define a [reusable, parameterized template](/blog/langchain-prompt-template/) for evaluating the helpfulness of a given text:
...
The [prompt instruction](/blog/prompt-engineering-best-practices/) defines what “helpfulness” means and provides scoring criteria. It also inserts the user-provided `text` using `f-string` formatting.
...
text_to_evaluate = "To improve your productivity, you could try using a time-blocking method. Break your day into segments and assign specific tasks to each segment."

evaluations: list[Eval] = []
for model in models:
    @llm.call(model, format=Eval)
    def evaluate(text: str) -> str:
        return evaluate_helpfulness(text).text()

    response = evaluate(text_to_evaluate)
    evaluations.append(response.parse())
```

This involves iterating through a list of models and creating a call for each.

Mirascope's `@llm.call` decorator lets you easily switch providers using minimal boilerplate since it abstracts away the details of working with their APIs.

We support a range of [popular providers](/docs/learn/llm), like OpenAI, Anthropic, Google, Mistral, and more.
...
Each LLM call uses a Mirascope **response model** to [structure](/blog/langchain-structured-output/) and validate the language model’s output, ensuring it conforms to the specified fields and types defined in `Eval`. This provides high-quality responses for downstream processing, such as using the output for another [prompt](/blog/prompt-engineering-examples/).
...
![Autcomplete Suggestion For Eval Reasoning Field](/assets/blog/llm-as-judge/autocomplete.webp)

## Scale Your Evaluation Process with Confidence

Mirascope lets you scale and optimize LLM-as-a-judge evaluations with minimal overhead and maximum flexibility. You can build [LLM-driven applications](/blog/llm-applications/) with our Python library offering simple, provider-agnostic tooling.

Want to learn more about how Mirascope can help you simplify and scale the evaluation process? You can find Mirascope code samples both in our [documentation](/docs) and on [GitHub](https://github.com/mirascope/mirascope/).
