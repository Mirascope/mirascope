...
space](/blog/prompt-engineering-tools)? Is it because people are bringing experimental stuff into production too quickly? Or simply because the space is so new?
...
An example of this is our [`@llm.call`](/docs/learn/llm/calls) decorator, which encapsulates as much logic within the prompt as feasible.

Within a decorated function, the return value can be used as the prompt template. The following example requests book recommendations based on a particular genre:
...
# > Sure! I recommend **"The Name of the Wind" by Patrick Rothfuss**. It's ...

```

By default, Mirascope treats a returned string as a single user message in order to simplify initial use and implementation for straightforward scenarios.
...
# > As a librarian, I'd recommend "The Name of the Wind" by Patrick Rothfuss...

```

Mirascope will automatically structure the prompt as a list of `BaseMessageParam` message objects that work across all supported providers.

The `@llm.call` decorator turns prompt templates into actual calls to an LLM API. This reduces the boilerplate needed to write LLM-powered functions:
...
In the above example, we've directly decorated the function with a specific provider and model. The `@llm.call` decorator is provider-agnostic—simply change the model string to switch providers:

```python
from mirascope import llm

@llm.call("openai/gpt-4o-mini")
def recommend_book_openai(genre: str) -> str:
    return f"Recommend a {genre} book"

@llm.call("anthropic/claude-sonnet-4-20250514")
def recommend_book_anthropic(genre: str) -> str:
    return f"Recommend a {genre} book"

# OpenAI call and response
openai_response = recommend_book_openai("fantasy")
print(openai_response.text())

# Anthropic call and response
anthropic_response = recommend_book_anthropic("fantasy")
...
Nevertheless, this approach to chaining encapsulates each step of the process within functions, allowing for a clean and readable way to sequentially execute tasks that depend on the outcome of previous steps:
...
def recommend_recipe(ingredient: str, food_type: str) -> str:
    ...
    return f"Imagine you are {chef.content}. Recommend a {food_type} recipe using {ingredient}"
...
response = recommend_books("coding")
print(response.text())
```
...
@llm.call("openai/gpt-4o-mini", format=Book)  # `format` not `response_model`
def recommend_book(genre: str) -> str:
    return f"Recommend a {genre} book"

try:
    response = recommend_book("fantasy")
    book = response.parse()  # Call .parse() to get the Book instance
...
@llm.call("openai/gpt-4o-mini", format=Literal["happy", "sad"])
def classify_sentiment(text: str) -> str:
    return f"Is the following text happy or sad? {text}"

def validate_happy(story: str) -> str:
    """Confirm that the story is happy."""
    sentiment = classify_sentiment(story).parse()
    assert sentiment == "happy", "Story wasn't happy"
    return story

@llm.call(
    "openai/gpt-4o-mini",
    format=Annotated[str, AfterValidator(validate_happy)],
)
def tell_sad_story() -> str:
    return "Tell me a very sad story."

try:
    response = tell_sad_story()
    story = response.parse()
    print(story)
except ValidationError as e:
    print(e)
    # > 1 validation error for ...
    #   Assertion failed, Story wasn't happy [type=assertion_error, ...]
```
...
For example, you can call `.stream()` on the decorated function to generate a stream of chunks:
...
You can also further configure the API call by passing parameters like `temperature` directly to the decorator, meaning that any time the decorated function is called it will use the correct, colocated call parameters:

```python
from mirascope import llm

@llm.call("openai/gpt-4o-mini", temperature=0.7)
def recommend_book(genre: str) -> str:
    return f"Recommend a {genre} book"
...
For providers with OpenAI-compatible APIs like [Ollama](https://ollama.com/), [Together](https://www.together.ai/), and others, you can use the built-in provider support or register a custom provider with a specific `base_url`:

```python
from mirascope import llm

# Ollama is built-in - just use the ollama/ prefix
...
For custom endpoints or providers not built-in, use `llm.register_provider()`:

```python
from mirascope import llm

# Register a provider with a custom base_url
llm.register_provider("openai", scope="custom/", base_url="http://your-endpoint/v1")

@llm.call("custom/model-name")
def recommend_book(genre: str) -> str:
    return f"Recommend a {genre} book"
```
...
Mirascope supports these [built-in LLM providers](/docs/learn/llm/providers):
...
If you want to switch to another model provider (like Anthropic or Google, for instance), you just need to change the model string:
...
Although LLMs are known mostly for text generation, you can provide them with specific tools (also known as [function calling](/docs/learn/llm/tools)) to extend their capabilities.
...
Mirascope lets you easily [define tools](/docs/learn/llm/tools#basic-usage-and-syntax) by documenting any function using a docstring as shown below. It automatically converts this into a tool, saving you additional work.
...
def get_current_weather(
    ...
    if "tokyo" in location.lower():
        ...
        return f"It is 22 degrees {unit} in Paris, France"
    else:
        return f"I'm not sure what the weather is like in {location}"

@llm.call("openai/gpt-4o", tools=[get_current_weather])
...
Mirascope supports Google, ReST, Numpydoc, and Epydoc style docstrings for creating tools. If a particular function doesn't have a docstring, you can wrap it with a tool function that provides the documentation:
...
See our [documentation](/docs/learn/llm/tools) for more details on defining and using tools.
...
To handle such scenarios, we support extraction with the [`format`](/docs/learn/llm/structured-output) argument in the decorator, which leverages tools (or optionally JSON mode) to reliably extract structured data from the outputs of LLMs according to the schema defined in a Pydantic `BaseModel`. In the example below you can see how due dates, priorities, and descriptions are being extracted:
...
@llm.call("openai/gpt-4o", format=TaskDetails)
...
# > {"output": [
#     "The Name of the Wind by Patrick Rothfuss – Follow the journey of Kvothe, a legendary figure and gifted magician, as he recounts his life story filled with adventure, magic, and mystery.",
#     "Mistborn: The Final Empire by Brandon Sanderson – Explore a world where the Dark Lord has won, and a group of rebels with unique metal-based powers attempt to overthrow the oppressive regime.",
#     "The Lies of Locke Lamora by Scott Lynch – Join Locke Lamora and his band of thieves in the bustling city of Camorr as they pull off daring heists, navigating danger and intrigue at every turn.",
# ]}
...
[Mirascope Cloud](/docs/learn/ops) lets you:

- Write prompts and LLM functions like you normally would
- Automatically version any change to the function with `@ops.version`
- Automatically trace every call to the function with `@ops.trace`
- Use `ops.instrument_llm()` to auto-trace all LLM calls
...
If you want to give Mirascope a try, we recommend taking a look at our [documentation](/docs). You can also find our source code on [GitHub](https://github.com/mirascope/mirascope). Don't forget to give us a star!
