...
v2_migration_completed: "2026-01-27"
---
...
# > Sure! I'd recommend "The Name of the Wind" by Patrick Rothfuss...
...
@llm.call("openai/gpt-4o-mini", prompt_template="Recommend a {genre} book")
...
@llm.call("openai/gpt-4o-mini", response_model=Book)
def recommend_book(genre: str) -> str:
    return f"Recommend a {genre} book"
...
@llm.call("openai/gpt-4o-mini", response_model=Literal["happy", "sad"])
...
For example, you can set `stream=True` in the function decorator, which will generate a stream of chunks:
...
@llm.call("openai/gpt-4o-mini", call_params={"temperature": 0.7})
def recommend_book(genre: str) -> str:
    return f"Recommend a {genre} book"
...
@llm.call("openai/gpt-4o-mini", client=client)
def recommend_book(genre: str) -> str:
    return f"Recommend a {genre} book"
...
Beyond OpenAI, Mirascope supports currently these [other LLM providers](/docs/learn/llm/calls):
...
If you want to switch to another model provider (like Anthropic or Mistral, for instance), you just need to change the model string:
...
See our [documentation](/docs/learn/llm/tools) for details on generating prompts in this way (for instance, by calling the `call` method).
...
To handle such scenarios, we support extraction with the [`response_model`](/docs/learn/llm/response-models) argument in the decorator, which leverages tools (or optionally `json_mode=True`) to reliably extract structured data from the outputs of LLMs according to the schema defined in a Pydantic `BaseModel`. In the example below you can see how due dates, priorities, and descriptions are being extracted:
