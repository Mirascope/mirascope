...
![Rag Diagram](/assets/blog/rag-llm-example/rag-diagram.webp)
...
* We also import Mirascope's `llm` module for working with language models
...
def ask_alan_turing(query: str, retriever: BaseRetriever) -> list:
...
* Receives a query string and our retriever object, and returns a list of messages to send to the LLM.
...
* Constructs a message list with a system message containing the excerpts and a user message with the query.

The `@llm.call` decorator turns the function into an LLM API call. It's provider agnostic: we could've instead written `"anthropic/claude-sonnet-4-20250514"` or `"google/gemini-2.0-flash"` with minimal code adjustments.

This illustrates the best practice of colocation of prompts and calls, which makes the code more readable as the logic for preparing input data, [building prompts](/blog/llm-prompt/), and LLM invocation is centralized in a single place.
