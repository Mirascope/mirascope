...
def query_documents(query: str, vector_store: Chroma):
    ...
    return f"""Here is some context related to a query: {context}
...
* Mirascope's `llm` module adds convenience around interactions with large language models.
...
Besides the query in the prompt you see below, note that we also retrieve relevant contextual information from the vector database to go along with our query, before sending everything together in our call to the model:
...
* We take the `page_content` attribute of each `Document` and include this as `context` in the returned prompt string.
* We call the model via our `@llm.call()` decorator.
* The function returns a formatted string that becomes the prompt sent to the model.
...
Want to learn more? You can find more Mirascope code samples both on our [docs](/docs) and on our [GitHub page](https://github.com/mirascope/mirascope).
