...
Similar to LlamaIndex, LangChain offers a number of [prompt templates](/blog/langchain-prompt-template) corresponding to different use cases:
...
An example of a `FewShotPromptTemplate` from [LangChain’s documentation](https://python.langchain.com/docs/how_to/few_shot_examples/):
...
Rather than attempt to tell you how you should formulate prompts, Mirascope provides its [`@llm.call`](/docs/learn/llm/calls) decorator so you can write prompt templates as standard Python functions.

The `@llm.call` decorator bundles a model with your prompt function. Simply return a string (which becomes the user message) or a list of messages, and call the function to get a response.

You can incorporate values computed inside the function into the prompt directly through f-strings.
...
In the example below, lists are formatted using Python's built-in `join` method:

```python
from mirascope import llm

@llm.call("openai/gpt-4o-mini")
def recommend_books(topics: list[str], genres: list[str]):
    topics_x_genres = [
        f"Topic: {topic}, Genre: {genre}" for topic in topics for genre in genres
    ]
    formatted = "\n".join(topics_x_genres)
    return f"""Recommend some books on the following topic and genre pairs:
{formatted}"""

response = recommend_books(["history", "science"], ["biography", "thriller"])
print(response.text())

# > Here are some book recommendations based on your topic and genre pairs:
# ### History, Biography
# - **"The Life of Samuel Johnson" by James Boswell**
# A classic biography that captures the complexities of one of history’s most fascinating literary figures.
# ### History, Thriller
# - **"The 39 Clues: The Maze of Bones" by Rick Riordan**
# Though aimed at a younger audience, this series combines historical facts with thrilling adventures and puzzles.
# ### Science, Biography
...
@llm.call("openai/gpt-4o-mini", format=ComplianceStatus)
def check_gdpr_compliance(procedure_text: str):
    return f"""Is the following data processing procedure compliant with GDPR standards?
    {procedure_text}"""

def validate_compliance(procedure_text: str) -> str:
    """Check if the data processing procedure is compliant with GDPR standards."""
    compliance_status = check_gdpr_compliance(procedure_text=procedure_text).parse()
    assert (
...
@llm.call("openai/gpt-4o-mini", format=DataProcessingProcedure)
def write_procedure():
    return """Write a detailed description of a data processing procedure.
    It MUST be compliant with GDPR standards."""

try:
    print(write_procedure().parse())
...
Mirascope also offers [Mirascope Cloud](/docs/learn/ops), which versions and traces every LLM function call and prompt automatically. By removing the worry of manually tracking changes systematically, Mirascope Cloud provides you the headspace to rapidly iterate on and optimize prompts, ensuring that all your fine-tuning efforts are recorded and retrievable.

Furthermore, Mirascope Cloud is also fully open-source and can be self-hosted.
...
LangChain offers similar abstractions to that of LlamaIndex for building data pipelines for [RAG applications](https://python.langchain.com/docs/tutorials/rag/). These abstractions correspond to two broad phases:
...
‍Mirascope provides additional methods for adding and retrieving documents to and from the vector store, as well as accessing the client store and index. You can find out more about these, as well as our integrations with LlamaIndex and [other frameworks](/blog/langchain-alternatives), in our documentation.
...
A prime example of this is Mirascope's support for [provider-specific parameters](/docs/learn/llm/calls), which are passed directly as keyword arguments to the call decorator:

```python
from mirascope import llm

@llm.call("openai/gpt-4o-mini", temperature=0.6)
...
Binding arguments to the model at runtime in such a way is the functional equivalent of Mirascope's direct keyword arguments (described above) and can be useful in situations where you want to take into account user interactions, in this case allowing users to mark a solution as being solved.
...
In the example below, chaining is implemented simply by calling each part of the chain inside of the parent function, passing intermediate results directly into the prompt:
...
@llm.call("openai/gpt-4o-mini")
def recommend_car(vehicle_type: str, criteria: str):
    brand_expert = get_brand_expert(vehicle_type=vehicle_type).text()
    return [
        llm.messages.system(
            f"""Imagine that you are the expert {brand_expert}.
            Your task is to recommend cars that you, {brand_expert}, would be excited to suggest."""
        ),
        llm.messages.user(
            f"Recommend a {vehicle_type} car that fits the following criteria: {criteria}."
        ),
    ]
