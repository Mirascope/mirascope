...
LLM agents are useful for different [real-world applications](/blog/llm-applications/) such as:
...
You can build agents using [modern orchestration frameworks](/blog/llamaindex-vs-langchain/) but many users find the unique abstractions of systems like LangChain hard to work with, requiring you to do things their way.
...
[LLM agents](/docs/learn/agents) are built around four key parts that work together to enable reasoning, planning, and execution.
...
![Agent Diagram](/assets/blog/llm-agents/agent-diagram.webp)
...
Popular techniques like [Chain-of-thought (CoT)](/docs/learn/llm) and Tree-of-thought (ToT) prompting or decision trees can help structure this process - enabling the agent to think through problems step by step, especially when dealing with complex multi-step tasks.  
...
Below are some of the most notable examples of [frameworks](/blog/llm-frameworks/):

### Mirascope

Mirascope lets you build autonomous or semi-autonomous systems that handle tasks, make decisions, and interact with users to integrate with tools, manage state, and even bring [humans into the loop](/docs/learn/agents) when needed.

Mirascope provides several key benefits, such as:

#### LLM Development in Native Python

Mirascope uses familiar Python constructs and avoids homegrown abstractions that other [frameworks](/blog/llamaindex-vs-langchain/#llamaindex-vs-langchain-vs-mirascope-an-in-depth-comparison) sometimes introduce. This allows developers with Python experience to start building right away without having to master new abstractions.

An example of such an abstraction is the [runnable](/blog/langchain-runnables/) in LangChain, which, though useful for creating and invoking custom chains,  often masks the underlying logic of how it works, making it harder for you to customize and debug.
...
We designate that function as a tool by listing it as a parameter in the `@llm.call` decorator:
...
@llm.call("openai/gpt-4o-mini", tools=[get_city_population])
def city_population(city: str):
    return f"What's the population of {city}"
...
Tool definitions are automatically generated at runtime from function signatures and docstrings.

Mirascope also embraces developer best practices like [colocation](/blog/engineers-should-handle-prompting-llms/), where function calls and their parameters - such as prompts, model parameters, and other configuration details are kept together within a single workflow.

You can see this in the code example above, where the `@llm.call` decorator encapsulates parameters such as the model name (`"openai/gpt-4o-mini"`) and tool definitions (`tools=[get_city_population]`) within the context of the `city_population` call.

Our prompt management tool [Mirascope Cloud](/docs/learn/ops) automatically versions and traces every LLM function call to ensure code readability and maintainability and to facilitate testing.

For instance, you can version an LLM call simply by adding the `@ops.version` decorator to ensure that any call the function makes is traced against the version:
...
Mirascope makes defining, structuring, and validating outputs easier with its `format` parameter, an extension of Pydantic's `BaseModel`.

Structured output handles output validation outside of runtime and adds type safety under the hood, allowing you to catch errors earlier in development.

In this example, we define the expected structure of the language model's response (`Movie`) and add this as a format in the LLM call to enforce its structure in order to more easily validate the model's [output](/blog/langchain-structured-output/):
...
def extract_movie(text: str):
    return f"Extract {text}"

response = extract_movie("Inception directed by Christopher Nolan")
...
![Mirascope Response Type Hints](/assets/blog/llm-agents/type-hints.webp)

It also provides autosuggestions:  

![Mirascope Autocomplete Suggestion](/assets/blog/llm-agents/autocomplete.webp)
...
Additionally, CrewAI integrates with various LLMs, including [LangChain’s ecosystem](/blog/langchain-sucks/). This means you can leverage LangChain’s tools and resources alongside CrewAI to enhance your agent workflows.
...
You can address some of these issues by [fine-tuning](/blog/prompt-engineering-vs-fine-tuning/) the model for specific tasks or combining it with RAG systems to fetch more relevant, up-to-date information.
...
The effectiveness of an LLM agent depends heavily on how well it’s [prompted](/blog/prompt-engineering-best-practices/). If a prompt lacks detail, structure, or specificity, the agent may produce responses that fail to address the intended goal.

Since agents don’t possess inherent reasoning capabilities to interpret or refine unclear instructions, users have to invest effort and care in [creating precise and well-thought-out prompts](/blog/prompt-engineering-examples/).
...
A typical [LLM pipeline](/blog/llm-pipeline/) uses retrieved data or documents to answer queries but here we’re adding an agent to decide on whether retrieval is needed in the first place.
...
You can find more details about the code below in our [tutorial library](/docs/learn/agents).
...
1. The `_call()` method composes a system/user prompt that includes the context retrieved using `get_documents`.
...
# Register ollama provider for local LLM
...
class OnboardingBot(BaseModel):
    def _call(self, question: str):
        ...
        @llm.call("ollama/llama3.1")
        def call_llm():
            return [
                llm.messages.system(f"""You are a helpful AI that is an expert at answering questions
about the company's onboarding docs. Here is relevant context.

Context:
{context}"""),
                llm.messages.user(f"{question}\n\nPlease provide a helpful, concise answer."),
            ]
...
Although we’ve included only one tool (`get_documents`), adding [multiple tools](/blog/openai-function-calling/) (e.g., a search function, a calculator, or an API call to other LLMs or agents) would let your agent choose the best action to perform before finalizing its answer.
...
Want to learn more? You can find Mirascope code samples both on our [documentation site](/docs) and on our [GitHub page](https://github.com/mirascope/mirascope).
