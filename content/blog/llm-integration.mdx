...
That's why we created [Mirascope](https://github.com/mirascope/mirascope/), a user-friendly toolkit for integrating LLMs into applications, and [Mirascope Cloud](/docs/learn/ops), our prompt management and observability platform.
...
An interesting detail about this case study of using [LangChain RAG](/blog/langchain-rag/) is its discussion of the different model capabilities and costs. For example, although OpenAI offered streaming while Vertex didn’t (at the time of the write-up), they still went with Vertex after extensive testing due to its “exceptional ability” to answer questions concisely and its easy authentication process.

Depending on the use case, teams may also consider fine-tuning a model to further improve performance, but this often involves additional costs and complexity, and as this [RAG LLM example](/blog/rag-llm-example/) shows, a well-structured retrieval system combined with prompt engineering can often achieve strong results without the need for fine-tuning.

(For more information and a tutorial on chatbots, see our guide on [how to make a chatbot](/blog/how-to-make-a-chatbot/).)
...
While frameworks provide pre-built abstractions that help integrate LLMs, they also have certain downsides **that we built Mirascope and Mirascope Cloud to address**.

Below, we present a few of these challenges and explain how we designed Mirascope and Mirascope Cloud to overcome them.
...
response = overview("Describe quantum computing")
print(response.text())
```

* We replace LangChain's `PromptTemplate` object with a simple f-string returned from the function, removing the need for explicit template construction and chaining.
* Instead of `RunnablePassthrough()`, we use Python native arguments as there's no explicit need for a passthrough.
* A manual LLM call and parsing (`ChatOpenAI()` and `StrOutputParser` respectively) are replaced by a single decorator `@llm.call("openai/gpt-4o-mini")`.
...
The call decorator injects function arguments into the formatted prompt, removing the need for manually formatting or passing inputs into the LLM call.
...
An example of this is shown below. To switch from OpenAI to Anthropic, you can use `llm.model()` to override provider settings at runtime to specify a different model.

```python {4 12-17}
from mirascope import llm

@llm.call("openai/gpt-4o-mini") # Calls GPT-4o-mini
...
Mirascope offers its Pydantic-based `format` parameter, allowing you to easily define, structure, and validate outputs using Pydantic's `BaseModel`.
...
@llm.call("openai/gpt-4o", format=WeatherResponse)
def get_weather(location: str) -> str:
    return f"What's the weather in {location}"

response = get_weather("Paris")
weather = response.parse()
print(weather.temperature, weather.condition)  # Fully structured, type-safe output!
```
...
![Mirascope Type Hints](/assets/blog/llm-integration/mirascope_type_hints.webp)

It also shows auto-suggestions:

![Mirascope Autocomplete](/assets/blog/llm-integration/mirascope_autocomplete.webp)
...
[Mirascope Cloud](/docs/learn/ops) versions and traces your LLM code automatically with every change you make.

You just add the `@ops.version` decorator to any function to start automatically versioning any changes that occur:
...
response = answer_question("What's the famous question that's answered by '42'?")
print(response.text())
```

Mirascope Cloud versions LLM generations using their function closure, so even changes to functions or classes used within the generation are automatically versioned (as long as they're user defined).

By adding `ops.configure()` and `ops.instrument_llm()` to the application (see code above), you also automatically trace all calls made by any function decorated with `@ops.version`. This means every call to an LLM is traced against the version of the code that generated it.

![Mirascope Cloud Trace Details Screenshot](/assets/blog/llm-integration/lilypad_generation_details_screenshot.webp)

Mirascope Cloud shows traces in real time as LLM calls are made, providing real-time summaries of every call so you can evaluate and improve prompts more quickly.
...
(If you also need to model complex relationships among your data, you can learn [how to build a knowledge graph](/blog/how-to-build-a-knowledge-graph/) to power more advanced querying and insights.)
...
Want to learn more about Mirascope's tools for building generative AI agents? You can find Mirascope code samples both in our [documentation](/docs) and our [GitHub repository](https://github.com/mirascope/mirascope).

Mirascope Cloud code samples can also be found in our [documentation](/docs/learn/ops). These resources can help you incorporate automation into your LLM integration workflow, speeding up deployment and reducing manual tasks.
