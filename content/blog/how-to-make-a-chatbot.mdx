...
All you need to do in this case is change the model string and nothing else, as Mirascope's call decorator abstracts away the complexities of provider-specific APIs, ensuring your function logic remains intact.

### 2. Simplifies Chatbot Development Through Clean Architecture

[LLM orchestration frameworks](/blog/llm-frameworks/), though written in Python or JavaScript, sometimes use homegrown abstractions that resemble data science workflows and neglect native structures already available in those languages.

Such abstractions seem novel at first but quickly become opaque the moment your code (e.g., your [chained calls](/blog/prompt-chaining/)) grows in complexity.

A case in point is [LangChain’s runnable](/blog/langchain-runnables/), which is a protocol for creating and invoking custom chains and sequencing calls (using the output of one as input to the next).
...
For example, a corporate chatbot might pass a user query such as “What’s the status of my expense reimbursement request?” through a [pipeline](/blog/llm-pipeline/) defined using `runnable`, where `RunnablePassthrough` forwards the query unchanged, a prompt formats the query for processing, and `runnable.bind(stop="END")` configures the language model to stop generating a response once it encounters the keyword "END:”
...
The above code uses a Python function decorated with `@llm.call` to directly define LLM interactions, avoiding intermediary abstractions like operators or runnables and simplifying the logic into a single function.

### 3. Offers Automatic Prompt Versioning

Building an LLM-driven chatbot involves [prompt engineering](/blog/advanced-prompt-engineering/), which helps define the chatbot’s behavior, context, and output style.
...
[Mirascope Cloud](/docs/learn/ops), which works with Mirascope, is an open source prompt engineering framework that automatically versions your code every time you make an LLM call. You can use Mirascope Cloud by adding the `@ops.version` decorator to any Python function:
...
Mirascope Cloud supports collaboration between technical and non-technical users via its Playground.
...
![Mirascope Cloud Prompt Editor](/assets/blog/how-to-make-a-chatbot/lilypad-screenshot.webp)
...
But it’s doing the next best thing: it feeds your query to the LLM, which decides which function or tool to call, and which responds with JSON schema (or some other [structured output](/blog/langchain-structured-output/)) for the function call to make for, e.g., the Bing Web Search API.

This piece of JSON is an *instruction* (including arguments and parameters) for how a function would call the API.

All this might seem a little complex, so if you intend on extending the functionality of your chatbot with [tools](/blog/openai-function-calling/) you might consider using a library that simplifies the process where feasible.
...
Mirascope automatically generates the necessary function calling definitions from functions decorated with `@llm.tool`.

You can specify the tool you want to use in your [LLM application](/blog/llm-applications/) by defining function parameters with type hints, which become the parameters of the tool.

Mirascope uses the function definition to automatically generate the JSON schema needed for tool calling.

It can also generate definitions from any docstring you specify in that function, if you use Google-, ReST, Numpydoc-, or Epydoc-style docstrings.

The following example demonstrates a custom tool (`get_art_gallery_info`) that fetches art gallery information using static logic, comparing gallery names against predefined cases like "The Louvre" or "The Metropolitan Museum of Art."

If the user asks "Tell me about the Louvre," the LLM uses the tool to formulate a precise response by invoking the `get_art_gallery_info` tool, which retrieves specific information about the Louvre and then integrates this information into the final response provided to the user:
...
def get_art_gallery_info(gallery: str) -> str:
    ...
    Args:
        gallery: The name of the art gallery, e.g., The Louvre.
    """
...
@llm.call("openai/gpt-4o", tools=[get_art_gallery_info])
def art_gallery_info(gallery: str):
    return f"Tell me about the art gallery {gallery}"
...
For example, below we define a function with validated inputs using Pydantic's `validate_call` decorator.

The moment this function is called, values for the parameters `genre` and `audience` will be constrained by the type annotations and validation rules, such as required types (`str`), minimum length (`min_length=3` for `genre`), and the presence of mandatory fields.
...
@llm.call("openai/gpt-4o-mini")
@validate_call  # ensure the arguments are validated
def recommend_book(genre: Annotated[str, Field(min_length=3)], audience: str):
...
Mirascope also validates LLM outputs via its **structured output** feature, which uses Pydantic's `BaseModel` to ensure outputs adhere to a predefined structure.

As shown below, you define a `format` as a Pydantic `BaseModel` and add it to an LLM call to validate the output.
...
@llm.call("openai/gpt-4o-mini", format=WeatherDetails)
def extract_weather(text: str):
    return f"Extract the weather details from this description: {text}"

response = extract_weather(text="It's sunny with a temperature of 75°F")
weather = response.parse()
print(weather.temperature)  # Output: 75°F

print(weather.condition)  # Output: sunny
```

This ensures the output conforms to the structure and types (fields) defined in `WeatherDetails`.

Note that the structured output format implicitly defines and enforces the return type from `.parse()`, which you can verify in your IDE before runtime to make sure everything's correct before sending the prompt:

![Mirascope Type Hint For Response](/assets/blog/how-to-make-a-chatbot/type-hints.webp)

It also shows auto-suggestions:

![Mirascope Type Hint For Response](/assets/blog/how-to-make-a-chatbot/autocomplete.webp)
...
These steps are based on [Mirascope's tutorial](/docs/learn/llm) where we show you how to add even more advanced features to the chatbot.
...
* [Dynamic prompting](/blog/prompt-engineering-best-practices/), using Mirascope’s `@prompt_template` decorator to inject conversation history and the user’s query into the [prompt](/blog/llm-prompt/).  
* Streaming responses for a smoother user experience.  
* It runs as an interactive loop.

##### 1. Import Mirascope and Pydantic Modules

As you'll see later on, we use message builders to construct our messages and Pydantic's `BaseModel` to make sure the message history is always a list of specific kinds of messages for a better customer experience and in order to avoid errors.
...
# Attribute to store the conversation history, containing both user and helper messages
    ...
    def _call(self, question: str):
        ...
        def call_llm():
            return [
                llm.messages.system("You are a useful helper."),
                *self.history,
                llm.messages.user(question),
            ]

        return call_llm.stream()
...
# Exit condition: If the user types "quit" or "exit", terminate the loop
     ...
     # Begin streaming the LLM's response in real-time
            ...
            print("")
            self.history.append(llm.messages.user(question))
            self.history.append(llm.messages.assistant(stream.text()))
...
* A list structure, `history`, to maintain our conversation.
* The `_call` method uses `@llm.call` decorator with `.stream()` to enable streaming.
* The message list specifies system and user messages. These roles guide the behavior of the chatbot in conversation, i.e., system defines the overarching behavior, tone, or rules that the chatbot follows and user represents input from the bot's user.
* The `run` method handles user input (quitting if the user types `quit` or `exit`) and calls the language model via `stream = self._call(question)`; the response is also streamed back chunk by chunk enabling real time display of the bot's reply and both the user's input and the bot's response are appended to the `history`.
...
##### 2. Define a `web_search` Tool

This tool leverages the DuckDuckGo Search API for retrieving and parsing information from webpages related to a specific query.
...
def web_search(query: str) -> str:
    ...
    try:
        ...
        results = DDGS(proxy=None).text(query, max_results=2)
...
* `web_search` creates a custom tool using Mirascope's `@llm.tool` decorator.
* The user's `query` is documented in the docstring, which Mirascope uses to generate the tool schema.
* The function retrieves web content and parses paragraph text. It returns a formatted string containing the content of the found webpages.
...
Below we add our web search tool to the parameter list of `@llm.call`, enabling the chatbot to do web searches when necessary.
...
class Chatbot(BaseModel):
    ...
    def _call(self, question: str | None = None):
        @llm.call("openai/gpt-4o-mini", tools=[web_search])
        def call_llm():
            return [
                llm.messages.system("""You are an expert web searcher.
...
provide as much information to the writer as possible without overwhelming them."""),
                *self.history,
                llm.messages.user(question) if question else None,
            ]
    ...
    def _step(self, question: str | None = None):
        ...
        if stream.tool_calls:
            tool_outputs = stream.execute_tools()
            # Add tool outputs as user message content for the next turn
            self.history.append(llm.messages.user(content=tool_outputs))
            return self._step()
        return stream.text()
...
Want to learn more about Mirascope's tools for building AI agents? You can find Mirascope code samples both in our [documentation](/docs) and our [GitHub repository](https://github.com/mirascope/mirascope).
