...
Meanwhile, [new tools and frameworks](/blog/langchain-alternatives) continue to appear in the space. So in this article, we’ll compare LangChain with two other tools that address some of its potential shortcomings:
...
LangChain offers a number of off-the-shelf [LCEL chain constructors](https://python.langchain.com/docs/versions/migrating_chains/) for different use cases, such as generating a SQL query from a question ([`create_sql_query_chain`](https://api.python.langchain.com/en/latest/chains/langchain.chains.sql_database.query.create_sql_query_chain.html#langchain.chains.sql_database.query.create_sql_query_chain)). The idea is you choose a constructor from their list and repurpose it with minimal hassle.
...
![Prompt Flow Visual](/assets/blog/prompt-flow-vs-langchain/prompt-flow-visual.webp)
...
def select_guide(region_type: str) -> str:
    return f"Name a travel guide who is very knowledgeable about {region_type} regions"


@llm.call("openai/gpt-4o-mini")
def recommend_destination(region_type: str, feature: str) -> list[llm.Message]:
    guide = select_guide(feature)
    return [
        llm.messages.system(
            f"Imagine that you are the guide {guide}. "
            f"Your task is to recommend travel destinations that you, {guide}, would be excited to showcase."
        ),
        llm.messages.user(f"Recommend a {region_type} destination that includes {feature}."),
    ]


response = recommend_destination(region_type="European", feature="historic sites")
print(response.text())
# > One fantastic European destination that brims with historic sites is Rome, Italy. Known as the "Eternal City ...
```
...
LangChain also allows you to [bind arguments](https://python.langchain.com/docs/how_to/binding/) (such as tools and model ID) to the call at runtime to dynamically specify call parameters. For example:
...
We’ve found that making the LLM call the central organizing unit around which everything gets versioned, including the prompt, [provides benefits](/blog/engineers-should-handle-prompting-llms) in terms of code cleanliness, maintainability, and scalability.
...
The way we typically do this is by passing all the parameters pertaining to the language model directly to the `@llm.call` decorator, as shown below:
...
The `@llm.call` decorator accepts configuration parameters directly, such as `tools=[get_weather]` to associate tools with the LLM call.
...
**Note**: Although not required, you can modify these settings in Mirascope at runtime without altering the general configuration. For instance, you can override the default parameters dynamically using the `llm.model()` context manager:
...
with llm.model("anthropic/claude-sonnet-4-5", temperature=0.5):
...
### Mirascope Cloud: Prompt Versioning and Management for Fine-Tuning LLM Inputs

Mirascope offers [Mirascope Cloud](/docs/learn/ops), which versions and traces every LLM call automatically. This is the necessary first step for prompt engineering. Without versioning and tracing, you lack the data to engineer prompts effectively.

We've purpose built Mirascope Cloud for prompt engineering to be simple and easy so you can take your mind off annoying and manual tedium and focus on what really matters: engineering quality prompts. In line with our commitment to open-source, Mirascope Cloud is fully open-source and can even be run locally.

To automatically version and trace LLM functions, you simply wrap the function in the `@ops.version` decorator:
...
Now the `recommend_book` function will be automatically versioned and traced on every call.

We also understand that not everyone uses Mirascope, so we've engineered Mirascope Cloud to work with provider SDKs directly:
...
ops.configure()
ops.instrument_llm()  # Auto-traces OpenAI calls  # [!code highlight]
client = OpenAI()
...
When using the `@ops.version` decorator, the version is determined based on all user-defined code that could impact the prompt. In more technical terms, the version is determined by the entire lexical closure.
...
![Mirascope Prompt Function Type Hints](/assets/blog/prompt-flow-vs-langchain/mirascope-type-hints.webp)

Autocompletion of a function call:

![Mirascope Prompt Function Autocomplete](/assets/blog/prompt-flow-vs-langchain/mirascope-autocomplete.webp)
...
Want to learn more? You can find more Mirascope code samples on both our [docs](/docs) and on [GitHub](https://github.com/mirascope/mirascope/).
