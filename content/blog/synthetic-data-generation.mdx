...
But for [complex applications](/blog/llm-applications/) requiring high realism, such as image generation or natural language processing, more advanced techniques like deep learning models or LLMs would be preferred.
...
While the usual methods for generating synthetic data produce datasets that are statistically accurate, they often lack the detailed complexities and domain-specific [knowledge](/blog/how-to-build-a-knowledge-graph/) needed to represent real-world scenarios effectively.
...
Below, we walk you through an example of using OpenAI's GPT-4o-mini to create ecommerce data on home appliances, including attributes like name, price, and inventory.

The main libraries we’ll use are:

* Mirascope for [prompting](/blog/advanced-prompt-engineering/), output parsing, and reliable validation  
...
class ProductData(BaseModel):
    """A single product data point."""

    product_name: str = Field(description="The name of the fashion product")
    price: float = Field(description="The price in dollars (including cents)")
    stock: int = Field(description="The quantity available in stock")


class DataFrameGenerator(BaseModel):
    """Generated data that can be converted to a DataFrame."""

    data: list[ProductData] = Field(
        description="The data to be inserted into the dataframe"
    )

    def generate_dataframe(self) -> pd.DataFrame:
        return pd.DataFrame([item.model_dump() for item in self.data])

    def append_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        return pd.concat([df, self.generate_dataframe()], ignore_index=True)


@llm.call("openai/gpt-4o-mini", format=DataFrameGenerator)
def generate_df_data(num_datapoints: int) -> str:
    return f"""
    Generate {num_datapoints} random but realistic datapoints of items which could
    be listed in an e-commerce fashion store.
    """


response = generate_df_data(5)
df_data = response.parse()
...
The function body provides the prompt with formatting instructions for the LLM, and the `@llm.call` decorator defines both the language model to use and the formatting of the output return type via the `format` parameter, which is specified by the Pydantic `BaseModel` we defined earlier. We then call `.parse()` on the response to get the structured output.

Mirascope’s call decorators easily turn any Python function into a prompt and ensure [colocation](/blog/engineers-should-handle-prompting-llms/) of both prompts and their calls.

Colocation simplifies development by keeping the logic, structure, and [prompt](/blog/llm-prompt/) content all in one place, improving readability, maintainability, and reducing context-switching for developers.

This differs from practices encouraged by other major [LLM frameworks](/blog/llm-frameworks/) that don’t promote colocation, with the effect that prompts, logic, and configuration can get scattered around the codebase, leading to greater challenges in debugging, testing, and iterating on functionality.

Mirascope's call decorators are also model agnostic, requiring minimal code changes for switching LLM providers (our library interfaces with [many popular providers](/docs)).

Mirascope's `format` parameter lets you take full advantage of Pydantic's type annotations, which integrate features like autocomplete, type hints, and linting directly in your IDE.

These allow you to catch potential errors like mismatched data types or missing fields before you even run your code:

![Mirascope Linting](/assets/blog/synthetic-data-generation/linting.webp)

We also offer auto suggestions:

![Mirascope Autocomplete](/assets/blog/synthetic-data-generation/autocomplete.webp)
...
@llm.call("openai/gpt-4o-mini", format=DataFrameGenerator)
def generate_additional_df_data(num_datapoints: int, df: pd.DataFrame) -> str:
    return f"""
    Generate {num_datapoints} random but realistic datapoints of items which would
    make sense to add to the following dataset:
    {df}
    """


response = generate_additional_df_data(5, df)
df_data = response.parse()
...
Besides imposing constraints for precise validation (as we did with `DataFrameGenerator`, by extending Pydantic’s `BasePrompt` class), we can also use [prompt](/blog/prompt-engineering-examples/) constraints to allow for natural language-defined rules.
...
@llm.call("openai/gpt-4o-mini", format=list[TV])
def generate_tv_data(num_datapoints: int) -> str:
    return f"""
...
response = generate_tv_data(10)
for tv in response.parse():
    print(tv)
```
...
![Chart Showing Desired Relationship In Data](/assets/blog/synthetic-data-generation/data-chart.webp)
...
In the last section, our [prompts](/blog/prompt-engineering-best-practices/) guided the model to generate outputs that specified TV price and size, aligning with domain-specific constraints.

We also [validated generated outputs](/blog/llm-evaluation/) by using Pydantic schemas, which enforce strict adherence to the defined structure, types, and constraints.
...
Want to learn more? You can find Mirascope code samples both on our [documentation site](/docs) and the [GitHub repository](https://github.com/mirascope/mirascope).
