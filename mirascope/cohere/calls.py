...
class CohereCall(BaseCall[CohereCallResponse, CohereCallResponseChunk, CohereTool]):
    ...
    def call(self, **kwargs: Any) -> CohereCallResponse:
        ...
        if self.call_params.logfire:
            chat = self.call_params.logfire(chat, "cohere")  # pragma: no cover
    ...
    async def call_async(self, **kwargs: Any) -> CohereCallResponse:
        ...
        if self.call_params.logfire_async:
            chat = self.call_params.logfire_async(chat, "cohere")  # pragma: no cover
        start_time = datetime.datetime.now().timestamp() * 1000
    ...
    def stream(self, **kwargs: Any) -> Generator[CohereCallResponseChunk, None, None]:
        ...
        if self.call_params.logfire:
            chat_stream = self.call_params.logfire(
                chat_stream, "cohere", CohereCallResponseChunk
            )  # pragma: no cover
