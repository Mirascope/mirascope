...
function flattenApiMeta(): void {
  const apiMetaPath = path.join(
    process.cwd(),
    ".docs-content/docs/api/_meta.json"
  );
  const mainMetaPath = path.join(
    process.cwd(),
    ".docs-content/docs/_meta.json"
  );
  ...
  const apiMeta: SectionSpec = JSON.parse(
    fs.readFileSync(apiMetaPath, "utf-8")
  );
  ...
  let apiItems: DocSpec[] = apiMeta.children;
  ...
  const mainMeta: FullDocsSpec = JSON.parse(
    fs.readFileSync(mainMetaPath, "utf-8")
  );
