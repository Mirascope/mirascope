...
from typing import Any, Awaitable, Callable, Literal, ParamSpec, TypeVar, overload
...
_BaseCallResponseT = TypeVar("_BaseCallResponseT", bound=BaseDynamicConfig)
...
def create_factory(
    *,
    ...
    def decorator(
        ...
        ) -> TBaseCallResponse | _ParsedOutputT:
            fn_args = get_fn_args(fn, args, kwargs)
            dynamic_config = fn(*args, **kwargs)
            create, prompt_template, messages, tool_types, call_kwargs = setup_call(
                model, client, fn, fn_args, dynamic_config, tools, call_params
            )
            start_time = datetime.datetime.now().timestamp() * 1000
            response = create(**call_kwargs)
            end_time = datetime.datetime.now().timestamp() * 1000
            output = TBaseCallResponse(
