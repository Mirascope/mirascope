...
const recommend_genre_prompt_template = llm.promptTemplate(
  ({ genre }: { genre: string }) => [
    llm.messages.user(`Recommend a ${genre} book`),
  ]
);

const recommend_genre_prompt_template_from_spec = llm.promptTemplate<{
  genre: string;
