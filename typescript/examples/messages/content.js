...
const textMessage = mirascope_1.llm.messages.user('Hello, how are you?');
const image = {
    type: 'image',
...
const imageMessage = mirascope_1.llm.messages.user([
...
const audioMessage = mirascope_1.llm.messages.user(audio);
const video = {
    type: 'video',
...
const videoMessage = mirascope_1.llm.messages.user([
...
const documentMessage = mirascope_1.llm.messages.user([
...
const toolCallMessage = mirascope_1.llm.messages.assistant([toolCall]);
const toolOutput = {
    type: 'tool-output',
    id: 'call-weather-123', // matches the tool call ID
    value: { temperature: 72, condition: 'sunny', humidity: 45 }, // output from the tool
};
const toolOutputMessage = mirascope_1.llm.messages.user([toolOutput]);
const thinking = {
    type: 'thinking',
...
const thinkingMessage = mirascope_1.llm.messages.assistant([
    thinking,
    "Based on your love for Andy Weir's problem-solving narratives, I recommend 'Recursion' by Blake Crouch.",
]);
const mixedMessage = mirascope_1.llm.messages.user([
