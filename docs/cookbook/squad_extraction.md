...
To extract an answer to a question, we can use the [`OpenAIChat.extract`](../api/chat/openai/models/openai_chat.md/#mirascope.chat.openai.models.OpenAIChat.extract) method to extract the answer. First, let's create a super basic schema and prompt for asking the question and extracting an answer:
