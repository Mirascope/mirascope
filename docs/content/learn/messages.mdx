...
<TabbedSection>
<Tab value="Python">
<CodeExample file="examples/messages/prompts.py" lines="25-35" />
</Tab>
<Tab value="Typescript">
...
Messages can contain much more than just text. Modern LLMs support multimodal content, allowing you to include multiple media types in a conversation, or even within a single message. Mirascope supports several content types, although [support varies depending on your chosen provider](/docs/compatibility).

- [`Text`](/docs/api/content#text)
- [`Image`](/docs/api/content#image)
- [`ImageUrl`](/docs/api/content#imageurl)
- [`Audio`](/docs/api/content#audio)
- [`AudioUrl`](/docs/api/content#audiourl)
...
[Depending on provider support](/docs/compatibility), you can supply `Image`, `Audio`, or `Document` media.

<TabbedSection>
<Tab value="Python">
<CodeExample file="examples/messages/content.py" lines="5-16" />
</Tab>
<Tab value="Typescript">
...
<Info>
**Media content** (`Image`, `Audio`) is represented by a `data` property, of type `str`. Generally, those strings are base64 encoded, although some document types are plain text. 

Media may also be specified by remote url using the `ImageUrl` and `AudioUrl` types. Specifying media by url can be more efficient, as it avoids the overhead of base64 encoding, and the LLM provider will download it directly from the external URL.

For convenience, the media types all have `from_file` and `from_bytes` as class methods.
...
`Document` content allows you to embed structured document files, such as PDFs. Like media content, it is specified by a string (base64 encoded or plain text depending on document type), but can be loaded using `from_file` and `from_bytes` class methods.


<TabbedSection>
<Tab value="Python">
<CodeExample file="examples/messages/content.py" lines="26-32" />
</Tab>
<Tab value="Typescript">
...
<TabbedSection>
<Tab value="Python">
<CodeExample file="examples/messages/content.py" lines="60-69" />
</Tab>
<Tab value="Typescript">
