...
Both approaches create identical messages, but using shorthand removes the need to wrap string messages in a `llm.context.Text` wrapper. We recommend using the shorthand functinos for convenience.
...
The most common type of content is text. It's represented as a `llm.Text` for consistency, but our APIs will automatically wrap plain strings:
...
In the next section on [Prompt Templates](/docs/learn/prompt-templates), we will describe how Mirascope provides a reusable functional interface for constructing prompts. This sets you up for success when context engineering your LLM [Calls](/docs/learn/calls).
