...
<Info>
**Media content** (`Image`, `Audio`, `Document`) are represented by a `data` property, of type `str`. Generally, those strings are base64 encoded, although some document types are plain text. 

Images may also be specified by http url using the `ImageUrl` type. Specifying an image by url can be more efficient, as it avoids the overhead of base64 encoding, and the LLM provider will download it directly from the external URL.

For convenience, the media types all have `from_file` and `from_bytes` as static class methods.
