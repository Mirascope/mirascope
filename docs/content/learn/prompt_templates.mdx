...
Working with Large Language Models (LLMs) involves providing them with "Prompts": lists of messages that provide all the context the LLM needs to produce a response. Engineering this context is the heart of AI engineering. Mirascope approaches this through Prompt Templates.
...
The above prompt is an example of the "functional style" of prompt templates, in which the prompt content is constructed directly in the prompt template's function body.
...
- **Base64 strings**: `"data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAYABgAAD..."`, `"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg=="`
...
However, if your prompt only involves a single user message, then you can write a "promptable" function that doesn't return a prompt, but rather returns `llm.Content` or `Sequence[llm.Content]`. In that case, you can use the `@prompt_template` decorator to "promote" your function into a prompt. 
...
It might look like `recommend_genre` returns a string, and `recommend_book_image` returns a list of content. However, after the decorator is applied, they both return `list[llm.Message]`—aka a `llm.Prompt`! In both cases, the prompt will consist of a single user message.
</Note>
...
<CodeExample file="examples/prompt_templates/functional.py" lines="61-74" />

### Message History Integration

You can also inject conversation history using list unpacking.

<CodeExample file="examples/prompt_templates/functional.py" lines="77-87" />


## Next Steps

By mastering prompts in Mirascope, you'll be prepared to start context engineering your prompts, resulting in robust and effective LLM powered applications.
