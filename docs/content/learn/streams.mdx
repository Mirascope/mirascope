...
When using the groups API, Mirascope automatically filters out the `ChunkStart` and `ChunkEnd` chunks for you. 

Each group provides:
- `type`: The content type for type discrimination (e.g., "text", "image", "audio")
- `partial`: Live-updating accumulated content as chunks are consumed, structured as a combined Chunk
- `final`: The complete content object once the group is finished (None until then). Note this will not be the chunk type, but the type of the final content, like `llm.Text`, `llm.Image`, and so forth.

Groups enforce sequential consumption - you must fully exhaust one group before moving to the next. This ensures proper ordering of multimodal content generation. If you attempt to iterate over a group before having exhausted the previous group, Mirascope will raise an exception.
...
`llm.ContentChunk` is an alias, which corresponds to five distinct chunk types. Each chunk type has a `type` field for type discrimination. Here are the types and their properties:
...
Async streams operate similarly to [async calls](/docs/learn/calls#async-calls). If you have a synchronous prompt, you can decorate it and call `stream_async()` to get an `llm.AsyncStream`:
...
If your prompt function itself is async, then `stream()` will be async, and will return an `llm.AsyncStream`:
...
Either way, you will get an `llm.AsyncStream` that supports async iteration. If your prompt is synchronous, then we recommend using `stream_async` for call-time flexibility. The main difference with an `llm.AsyncStream` is that you use `async for` to iterate through chunks (or groups).
### Async Groups
