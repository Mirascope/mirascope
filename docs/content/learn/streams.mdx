...
Each chunk provides `usage` and `cost` properties which give token usage and cost for that chunk (if available). The stream also keeps track of its running total `usage` and `cost`, as shown below:
...
Note how we wrap the iteration loop in a try/except block to catch any errors that might occur during streaming. The initial response when calling `.stream()` will return an iterable, but any errors that may occur during streaming will not happen until you actually iterate through it.
