...
<CodeExample file="examples/tools/loop.py" />
...
| Value | Behavior |
| --- | --- |
| `None` (default) | Uses the provider default (generally strict if well supported) |
| `True` | Uses strict schema validation, if supported |
...
<CodeExample file="examples/tools/parallel.py" />
...
When any tool is async, the call must be async too, and you use `await` with `execute_tools()` and `resume()`. See the [Async](/docs/mirascope/v2/async) guide for more details.
...
<TabbedSection>
<Tab value="@llm.call">
<CodeExample file="examples/tools/with_call.py" />
</Tab>
<Tab value="@llm.prompt">
<CodeExample file="examples/tools/with_prompt.py" />
</Tab>
<Tab value="Model">
<CodeExample file="examples/tools/with_model.py" />
</Tab>
</TabbedSection>
...
- [Streaming](/docs/mirascope/v2/streaming) — Stream responses with tool calls
- [Context](/docs/mirascope/v2/context) — Inject dependencies into tools
- [Agents](/docs/mirascope/v2/agents) — Build autonomous agents with tools
