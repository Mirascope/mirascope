...
class InitialSummaryPrompt(BasePrompt):
    prompt_template = """
    SYSTEM:
    Write a summary about the article that is long (4-5 sentences) yet highly 
    non-specific. Use overly, verbose language and fillers(eg.,'this article discusses') 
    to reach ~80 words.
    USER:
...
class RewrittenSummaryPrompt(InitialSummaryPrompt):
    prompt_template = """
    SYSTEM:
    Article: {article}
    ...
    USER:
    Here is the previous summary: {previous_summary}
    """
...
def summarize_article(article: str, summary_steps: int = 3):
    ...
    summary: InitialSummary = client.chat.completions.create(
        ...
        messages=InitialSummaryPrompt(article=article).messages(),
        max_retries=2,
    )
    ...
    for _i in range(summary_steps):
        new_summary: RewrittenSummary = client.chat.completions.create(
            model="gpt-4-0613",
            messages=RewrittenSummaryPrompt(
                article=article, summary_chain=summary_chain
            ).messages(),
            max_retries=5,
            max_tokens=1000,
